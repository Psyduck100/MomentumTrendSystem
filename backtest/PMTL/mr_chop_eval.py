...
def mr_rsi_signals_and_position(
    close: pd.Series,
    ...
    *,
    ...
    for i in range(len(close)):
        ...
        if not bool(valid.iat[i]) or not bool(chop.iat[i]):
            in_pos = False
            hold = 0
            continue
        ...
        if in_pos:
            hold += 1
            if (rsi.iat[i] > rsi_exit) or (hold >= max_hold_days):
                in_pos = False
                hold = 0
        else:
            if rsi.iat[i] < rsi_enter:
                in_pos = True
                hold = 0

    return pos, rsi, valid


def mr_quantile_signals_and_position(
    close: pd.Series,
    regime_chop: pd.Series,
    *,
    lookback: int,
    ...
    q_low = close.rolling(lookback, min_periods=lookback).quantile(q_enter)
    q_exit = close.rolling(lookback, min_periods=lookback).quantile(q_exit_q)
    valid = (~q_low.isna()) & (~q_exit.isna())
    ...
    for i in range(len(close)):
        ...
        if in_pos:
            hold += 1
            if (close.iat[i] > q_exit.iat[i]) or (hold >= max_hold_days):
                in_pos = False
                hold = 0
        else:
            if close.iat[i] < q_low.iat[i]:
                in_pos = True
                hold = 0

    return pos, q_low, valid
...
def apply_costs(
    ...
    *,
    ...
    if cost_bps_roundtrip and cost_bps_roundtrip > 0:
        ...
        strategy_ret = (
            strategy_ret - cost * (pos_diff == 1.0) - cost * (pos_diff == -1.0)
        )
...
def trade_list(
    ...
    for entry in entries:
        ...
        trade_ret = (
            float((1.0 + trade_slice).prod() - 1.0) if len(trade_slice) else np.nan
        )
...
def evaluate_mr_params(
    ...
    *,
    ...
    if strategy == "rsi":
        pos, _, _ = mr_rsi_signals_and_position(
            close,
            chop,
            rsi_n=int(params["rsi_n"]),
            rsi_enter=float(params["rsi_enter"]),
            rsi_exit=float(params["rsi_exit"]),
            max_hold_days=int(params["max_hold_days"]),
        )
    elif strategy == "quantile":
        pos, _, _ = mr_quantile_signals_and_position(
            close,
            chop,
            lookback=int(params["lookback"]),
            q_enter=float(params["q_enter"]),
            q_exit_q=float(params["q_exit_q"]),
            max_hold_days=int(params["max_hold_days"]),
        )
    else:
    ...
    strategy_ret = apply_costs(pos_exec, returns, cost_bps_roundtrip=cost_bps_roundtrip)

    pos_exec = pos_exec.astype(int)
    pos_diff = pos_exec.diff().fillna(0)
    trade_active = (pos_exec == 1) | (pos_diff != 0)
    ...
    trades = trade_list(pos_exec, strategy_ret)
    tstats = trade_stats(trades)

    occupancy_in_chop = float(pos_exec[chop].mean()) if chop.any() else np.nan
    occupancy_total = float(pos_exec.mean())
    entries = int((pos_diff == 1).sum())
    entries_per_year = float(
        entries / years_in_index(close.index, periods_per_year=periods_per_year)
    )
    chop_days = int(chop.sum())
    entries_per_chop_year = (
        float(entries / (chop_days / periods_per_year)) if chop_days > 0 else np.nan
    )

    inpos_mask = pos_exec.astype(bool)
    ann_over_chop_time = ann_return_over_mask(
        strategy_ret, chop, periods_per_year=periods_per_year
    )
    ann_over_inpos_time = ann_return_over_mask(
        strategy_ret, inpos_mask, periods_per_year=periods_per_year
    )

    return {
        "strategy": strategy,
        "lookback": int(params.get("lookback", -1)),
        "z_enter": float(params.get("z_enter", -1)),
        "z_exit": float(params.get("z_exit", -1)),
        "rsi_n": int(params.get("rsi_n", -1)),
        "rsi_enter": float(params.get("rsi_enter", -1)),
        "rsi_exit": float(params.get("rsi_exit", -1)),
        "q_enter": float(params.get("q_enter", -1)),
        "q_exit_q": float(params.get("q_exit_q", -1)),
        "max_hold_days": int(params.get("max_hold_days", -1)),
...
def sweep_params_rsi(
    close: pd.Series,
    regime_chop: pd.Series,
    *,
    rsi_ns: Iterable[int],
    rsi_enters: Iterable[float],
    rsi_exits: Iterable[float],
...
def sweep_params_quantile(
    close: pd.Series,
    regime_chop: pd.Series,
    *,
    lookbacks: Iterable[int],
    q_enters: Iterable[float],
    q_exit_qs: Iterable[float],
    max_hold_days_list: Iterable[int],
    cost_bps_roundtrip: float,
    periods_per_year: int = 252,
) -> pd.DataFrame:
...
def run_train_test(
    ...
    *,
    ...
    train_rsi = sweep_params_rsi(
        train_close,
        train_chop,
        rsi_ns=[2, 3],
        rsi_enters=[5, 10, 15, 20],
        rsi_exits=[40, 50, 60],
        max_hold_days_list=[3, 5, 10, 15, 30],
        cost_bps_roundtrip=cost_bps_roundtrip,
        periods_per_year=periods_per_year,
    )
    train_quant = sweep_params_quantile(
        train_close,
        train_chop,
        lookbacks=[20, 40, 60],
        q_enters=[0.05, 0.10, 0.15],
        q_exit_qs=[0.50, 0.60],
        max_hold_days_list=[5, 10, 30, 100],
        cost_bps_roundtrip=cost_bps_roundtrip,
        periods_per_year=periods_per_year,
    )
    test_rsi = sweep_params_rsi(
        test_close,
        test_chop,
        rsi_ns=[2, 3],
        rsi_enters=[5, 10, 15, 20],
    ...
    test_quant = sweep_params_quantile(
        test_close,
        test_chop,
        lookbacks=[20, 40, 60],
        q_enters=[0.05, 0.10, 0.15],
    ...
    train_df = pd.concat([train_rsi, train_quant], ignore_index=True)
    test_df = pd.concat([test_rsi, test_quant], ignore_index=True)
...
def combine_train_test(
    ...
    merged["robust_min"] = merged[
        ["train_chop_ann_return", "test_chop_ann_return"]
    ].min(axis=1)
    merged["robust_mean"] = (
...
def main():
    ...
    from backtest.PMTL.Test2 import build_regime
