...
def plot_up_down_regimes(
    ...
    *,
    ...
    # Shade UP and DOWN (different colors)
    _shade_runs(
        ax, close.index, reg["is_up"], color="green", alpha=0.16, label="UP regime"
    )
    _shade_runs(
        ax, close.index, reg["is_down"], color="red", alpha=0.16, label="DOWN regime"
    )

    # Optional: mark regime switches
...
def main():
    ...
    print("Regime counts (-1=down, 0=chop, 1=up):")
    ...
    is_up = reg["is_up"].reindex(close.index).fillna(False)
    strat_returns = gld_returns.where(is_up, 0.0)

    time_in_pos = float(is_up.mean())
    print("PMTL GLD (bull-only) metrics:", perf_metrics(strat_returns))
    print(f"Time in position (bull): {time_in_pos:.6f}")

    plot_up_down_regimes(
        close,
        reg,
        ma_len=feature_params_up["ma_len_entry"],
        logy=True,
        show_switch_markers=True,
    )

    # reg.to_csv("gld_regimes_up_down_chop.csv", index=True)
