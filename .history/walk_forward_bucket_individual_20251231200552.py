...
def main():
    ...
    results.sort(key=lambda r: (r[1]["median_sharpe"], r[1]["median_cagr"]), reverse=True)
    ...
    for bucket, stats in results:
        ...
        for fd in stats["fold_details"]:
            print(
                ...
                f"CAGR {'' if fd['cagr'] is None else f'{fd['cagr']*100:5.2f}%'} "
                f"Sharpe {'' if fd['sharpe'] is None else f'{fd['sharpe']:5.2f}'} "
                f"MaxDD {'' if fd['maxdd'] is None else f'{fd['maxdd']*100:6.2f}%'} "
                f"Turn {'' if fd['turnover'] is None else f'{fd['turnover']:5.2f}%'}"
            )
