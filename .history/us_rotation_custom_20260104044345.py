...
from momentum_program.analytics.constants import SCORE_MODE_RW_ALPHA
...
OBJECTIVE_GAMMA = 0.5
MAX_DRAWDOWN_TOLERANCE = 0.05  # strategy max drawdown cannot exceed benchmark by more than 5 percentage points
...
REFINED_ALPHA_GRID = [
    round(1.0 + 0.05 * i, 2) for i in range(int((2.0 - 1.0) / 0.05) + 1)
]
...
SCORE_CONFIGS = REFINED_ALPHA_CONFIGS

FILTER_MODES = ["ret_6m"]
RANK_GAPS = [1]
COARSE_COMBO_DEFS = build_combo_defs(SCORE_CONFIGS)

REFINED_ALPHA_RANK_GAPS = RANK_GAPS
REFINED_ALPHA_FILTERS = FILTER_MODES
...
def summarize_turnover(turnover_series: pd.Series) -> dict[str, float]:
    ...
    annual = (
        trimmed.groupby(trimmed.index.year).sum()
...
def run_walk_forward_splits(
    ...
    combo_defs = build_combo_defs(
        score_configs, rank_gaps=rank_gaps, filter_modes=filter_modes
    )
    print(
    ...
    for idx, block in enumerate(splits, start=1):
        ...
        for score_config, gap, filter_mode in combo_defs:
            ...
            if test_snapshot is not None:
                ...
                bench = test_snapshot.get("benchmarks", {}).get(OBJECTIVE_BENCHMARK)
                if bench and "max_drawdown" in bench:
                    gap_value = (
                        test_snapshot["metrics"]["max_drawdown"] - bench["max_drawdown"]
                    )
                    entry["test_drawdown_gaps"].append(gap_value)

        if not block_candidates:
            print(f"  Split {block_name}: insufficient data; skipped")
            continue

        test_candidates = [c for c in block_candidates if c.get("test")]
        if test_candidates:
            test_candidates.sort(
                key=lambda c: (
                    c["test"].get("excess_cagr")
                    if c["test"].get("excess_cagr") is not None
                    else c["test"]["metrics"]["cagr"]
                ),
                reverse=True,
            )
            winner = test_candidates[0]
        else:
        ...
        test_ranked = test_candidates if test_candidates else []
        ...
        if best_test is None:
            ...
            print(
                f"  Split {block_name}: score={winner['score']} gap={winner['rank_gap']} filter={winner['filter']} | "
                f"test excess CAGR {test_display*100:5.2f}% | train {winner_train_desc}"
            )
        ...
        aggregate_rows.append(
            {
                ...
                "avg_test_excess": avg_test_excess,
                "median_test_excess": _median(entry["test_excess"]),
                "p25_test_excess": _percentile(entry["test_excess"], 25.0),
                "avg_test_objective": _avg(entry["test_objectives"]),
                "avg_test_drawdown_gap": _avg(entry["test_drawdown_gaps"]),
                "train_count": entry["train_count"],
                "test_count": test_count,
                "test_top_hits": entry["test_top_hits"],
                "test_top_fraction": (
                    float(entry["test_top_hits"] / test_count) if test_count else 0.0
                ),
            }
        )
    ...
    aggregate_rows.sort(
        key=lambda row: (
            row["avg_test_excess"]
            if row["avg_test_excess"] is not None
            else float("-inf")
        ),
        reverse=True,
    )
...
def compute_plateau_selection(
    aggregate_rows: list[dict[str, Any]],
    drawdown_tolerance: float = MAX_DRAWDOWN_TOLERANCE,
    top_fraction: float = 0.2,
) -> dict[str, Any]:
    eligible = [
        row
        for row in aggregate_rows
        if row.get("avg_test_excess") is not None
        and row.get("avg_test_drawdown_gap") is not None
        and row.get("test_count", 0) > 0
        and row["avg_test_drawdown_gap"] <= drawdown_tolerance
    ]
    ...
    plateau_count = max(1, math.ceil(len(eligible) * top_fraction))
    plateau = eligible[:plateau_count]

    def complexity_key(row: dict[str, Any]) -> tuple[float, float, float]:
        param_type = row.get("score_param_type")
        if param_type is None:
            complexity = 0
        elif param_type == "lambda":
            complexity = 1
        else:
            complexity = 2
        param_value = row.get("score_param")
        param_abs = (
            abs(float(param_value))
            if isinstance(param_value, (int, float))
            else float("inf")
        )
        return (complexity, param_abs, -row["avg_test_excess"])

    plateau_winner = min(plateau, key=complexity_key)
    ...
    return {
        ...
        "plateau": plateau,
        "winner": plateau_winner,
    }


def run_walk_forward_analysis() -> dict[str, Any]:
    coarse_output = run_walk_forward_splits(
        SCORE_CONFIGS,
        WALK_FORWARD_SPLITS,
        label="coarse",
    )
    coarse_plateau = compute_plateau_selection(coarse_output["aggregate"]["per_config"])

    refined_output: dict[str, Any] | None = None
    refined_plateau: dict[str, Any] | None = None
    if REFINED_ALPHA_CONFIGS:
        refined_output = run_walk_forward_splits(
            REFINED_ALPHA_CONFIGS,
            WALK_FORWARD_SPLITS,
            label="refined_alpha",
            rank_gaps=REFINED_ALPHA_RANK_GAPS,
            filter_modes=REFINED_ALPHA_FILTERS,
        )
        refined_plateau = compute_plateau_selection(
            refined_output["aggregate"]["per_config"]
        )
    ...
    payload: dict[str, Any] = {
        ...
        "plateau": coarse_plateau,
        "refined": None,
    }

    if refined_output and refined_plateau is not None:
        payload["refined"] = {
            "score_configs": [
                {
                    "label": cfg.label,
...
def main() -> None:
                ...
                monthly_rows.append(
                    {
                        ...
                        "spy_return": (
                            float(spy_series.loc[timestamp])
                            if pd.notna(spy_series.loc[timestamp])
                            else None
                        ),
                        "qqq_return": (
                            float(qqq_series.loc[timestamp])
    ...
    if selected_full_result is not None:
        turnover_series = compute_turnover_series(
            selected_full_result["overall_returns"]
        )
