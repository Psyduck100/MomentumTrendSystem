...
def compute_blend_score(returns_df: pd.DataFrame) -> pd.Series:
    ...
    score = sum(
        w * returns_df[f"ret_{m}m"] for w, m in zip(LOOKBACK_WEIGHTS, LOOKBACK_MONTHS)
    )
    return score

def backtest_us_equities(prices: pd.DataFrame, monthly_returns: pd.DataFrame) -> dict:
    ...
    for i in range(1, len(monthly_returns)):
        # Compute blend_6_12 for US equities only (excluding IEF)
        blend_scores = compute_blend_score(monthly_returns.iloc[i])
        us_scores = blend_scores[UNIVERSE]
        
        # Check 12M filter
        ret_12m = monthly_returns.iloc[i]["ret_12m"]
        us_ret_12m = ret_12m[UNIVERSE]

        # Find best US equity by blend_6_12
        best_idx = us_scores.idxmax()
        best_12m = us_ret_12m[best_idx]

        # Apply ret_12m filter
        if best_12m > 0:
            position = best_idx
        else:
...
def main():
    ...
    result = backtest_us_equities(prices, monthly_returns)
