...
def main() -> None:
    ...
    for start_date, end_date in windows:
        ...
        for bucket, benchmark_ticker in BENCHMARK_MAP.items():
            if (
                bucket in data_rw["bucket_returns"]
                and not data_rw["bucket_returns"][bucket].empty
            ):
                bucket_cagr = compute_metrics(
                    data_rw["bucket_returns"][bucket]["return"]
                )["cagr"]
                bench_cagr = compute_benchmark_cagr(
