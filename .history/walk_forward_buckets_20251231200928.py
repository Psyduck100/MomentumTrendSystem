...
def run_walk_forward_bucket_comparison() -> None:
    ...
    print(
        f"Universe: {len(tickers)} tickers across {len(all_buckets)} buckets (REITs removed)\n"
    )
    ...
    for config_name, buckets_to_include in configs.items():
        ...
        for fold_idx, fold in enumerate(folds, 1):
            ...
            if (test_start_dt.year - train_start_dt.year) * 12 + (
                test_start_dt.month - train_start_dt.month
            ) < warmup_months:
                print(
                    f"  Fold {fold_idx} ({fold['test_start'][:4]}): skipped (insufficient warmup)"
                )
                continue
    ...
    for config_name, stats in results.items():
        print(f"\n{config_name}:")
        print(
            f"{'Test Period':<20} {'Sharpe':<10} {'CAGR':<10} {'MaxDD':<10} {'Turnover':<10}"
        )
        print("-" * 75)
