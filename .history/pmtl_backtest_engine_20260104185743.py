...
class PMTLBacktestEngine:
    ...
    def __init__(
        self, main_ticker: str, start_date: str, end_date: str, frequency: str = "ME"
    ):
    ...
    def _download_prices(self):
        ...
        data = yf.download(
            self.main_ticker, start=self.start_date, end=self.end_date, progress=False
        )

        # Handle both single and multi-ticker downloads
        if isinstance(data.columns, pd.MultiIndex):
            self.prices = (
                data[("Close", self.main_ticker)]
                if ("Close", self.main_ticker) in data.columns
                else data.iloc[:, 0]
            )
        else:
            self.prices = (
                data["Close"] if "Close" in data.columns else data["Adj Close"]
            )
    ...
    def backtest_sma(self, window: int, fallback) -> pd.Series:
        ...
        """
        # Calculate SMA with proper warmup; no trades until window is populated
        ma = self.prices.rolling(window=window, min_periods=window).mean()

        # Get period-end prices and dates
        monthly_prices = self.prices.resample(self.frequency).last()
        monthly_dates = monthly_prices.index

        # Get MA values at period end
        ma_at_month_end = ma.reindex(monthly_dates, method="ffill")

        # Generate signal from prior period info: compare price vs MA, then shift
        signal = (monthly_prices > ma_at_month_end).astype(int).shift(1)

        # Debug: print sample of price, MA, and signal for first 12 months
        print(f"\n[DEBUG] SMA window {window} sample (first 12 months):")
        debug_df = (
            pd.DataFrame(
                {
        ...
        # Calculate main asset returns at rebalance frequency
        main_monthly_ret = monthly_prices.pct_change()

        # Get fallback returns aligned to rebalance dates
        fallback_ret = fallback.get_monthly_returns(monthly_dates).reindex(
            monthly_dates
        )

        # Align everything by index to avoid silent misalignment
        df = pd.concat([main_monthly_ret, fallback_ret, signal], axis=1)
        df.columns = ["main", "fallback", "signal"]
        df = df.dropna()

        blended_ret = df["signal"] * df["main"] + (1 - df["signal"]) * df["fallback"]

        # Debug: log how often fallback is used for this window
        fallback_count = (df["signal"] == 0).sum()
        main_count = (df["signal"] == 1).sum()
        total = len(df)
        print(
            f"[PMTLBacktestEngine] SMA window {window}: fallback used {fallback_count}/{total} periods ({fallback_count/total:.1%}), main asset held {main_count}/{total} periods ({main_count/total:.1%})"
        )

        return blended_ret.rename(f"SMA_{window}")
    ...
    def backtest_ema(self, window: int, fallback) -> pd.Series:
        ...
        Returns:
            Series of period returns (monthly/weekly/biweekly)
        """
        # Calculate EMA; mask early warmup so signals don't trade too early
        ma = self.prices.ewm(span=window, adjust=False).mean()
        if window > 1:
            ma.iloc[: window - 1] = pd.NA
        ...
        return blended_ret.rename(f"EMA_{window}")

    def backtest_trailing_return(self, periods: int, fallback) -> pd.Series:
        """Absolute momentum on trailing return: if trailing return > 0, hold main; else fallback."""

        monthly_prices = self.prices.resample(self.frequency).last()
        monthly_dates = monthly_prices.index

        # Trailing return over the lookback window (e.g., 6m, 12m)
        trailing_ret = monthly_prices.pct_change(periods=periods)

        # Signal uses prior period info
        signal = (trailing_ret > 0).astype(int).shift(1)

        # Main and fallback returns
        main_monthly_ret = monthly_prices.pct_change()
        fallback_ret = fallback.get_monthly_returns(monthly_dates).reindex(
            monthly_dates
        )
        ...
        return blended_ret.rename(f"RET_{periods}m")
    ...
    def run_window_sweep(
        self, fallback, windows: list, use_sma: bool = True, use_ema: bool = True
        ...
        benchmark_annual = benchmark_rets.groupby(benchmark_rets.index.year).apply(
            lambda x: (1 + x).prod() - 1
        )
        ...
        if use_sma:
            ...
            for window in windows:
                ...
                metrics = compute_metrics(sma_rets, periods_per_year)
                ...
                annual = sma_rets.groupby(sma_rets.index.year).apply(
                    lambda x: (1 + x).prod() - 1
                )
                ...
                print(
                    f"  SMA {window:3d}: CAGR {metrics['cagr']:6.2%}, "
        ...
        if use_ema:
            ...
            for window in windows:
                ...
                metrics = compute_metrics(ema_rets, periods_per_year)
                ...
                # Annual returns (compound monthly inside each year)
                annual = ema_rets.groupby(ema_rets.index.year).apply(
                    lambda x: (1 + x).prod() - 1
                )
                ...
                print(
                    f"  EMA {window:3d}: CAGR {metrics['cagr']:6.2%}, "
                    f"Sharpe {metrics['sharpe']:6.3f}, MaxDD {metrics['max_drawdown']:7.2%}"
                )
