...
def main() -> None:
    parser = argparse.ArgumentParser(description="Exclude top years and recompute stats")
    ...
    if args.exclude_years:
        ...
        df = df[~df.get("test_year", pd.Series(dtype=int)).isin(years)]
    ...
    print(f"CAGR: {stats['cagr']*100:.2f}% (95% CI {stats['cagr_ci_low']*100:.2f}% – {stats['cagr_ci_high']*100:.2f}%)")
