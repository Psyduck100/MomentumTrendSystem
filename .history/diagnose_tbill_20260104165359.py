...
# Load data
gld = yf.download('GLD', start='2005-01-01', end='2025-12-31', progress=False)['Adj Close']
...
print(f"GLD monthly dates range: {monthly_gld.index.min()} to {monthly_gld.index.max()}")
...
tbill_monthly = tbill_df.loc[tbill_df.index.to_period('M').isin(
    monthly_gld_ret.index.to_period('M')), 'TB3MS_monthly_ret']
print(f"\nTB3MS aligned to monthly GLD: {len(tbill_monthly)} values")
print("First 10:")
print(tbill_monthly.head(10))

# Check the blend - 100% TB3MS (when signal=0)
tbill_only = tbill_df.loc[tbill_df.index.to_period('M').isin(
    monthly_gld.index.to_period('M')), 'TB3MS_monthly_ret']
