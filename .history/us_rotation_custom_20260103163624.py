...
RANK_GAPS = [0, 1, 2]
COMBO_DEFS = list(product(SCORE_MODES.items(), RANK_GAPS, FILTER_MODES))
...
SELECTED_CONFIG = {"score": "blend_6_12", "rank_gap": 0, "filter": "ret_12m"}
...
def run_walk_forward_analysis() -> dict[str, Any]:
    ...
    wf_payload: dict[str, Any] = {"config": None, "train": None, "test": None}
    ...
    for (score_label, score_mode), gap, filter_mode in COMBO_DEFS:
        ...
        metrics = compute_metrics(overall["return"])
        if best_config is None or metrics["cagr"] > best_config["metrics"]["cagr"]:
            best_config = {
    ...
    train_metrics = {k: float(v) for k, v in best_config["metrics"].items()}
    wf_payload["config"] = {
        "score": best_config["score_label"],
    ...
    print(
        ...
        f"score={best_config['score_label']} gap={best_config['rank_gap']} filter={best_config['filter']} | "
    ...
    test_result = backtest_momentum(
        ...
        top_n_per_bucket=1,
        rank_gap_threshold=best_config["rank_gap"],
        score_mode=best_config["score_mode"],
        abs_filter_mode=best_config["filter"],
        abs_filter_band=0.0,
        abs_filter_cash_annual=0.04,
        cache_dir=BACKTEST_CACHE,
        slippage_bps=3.0,
    ...
    test_metrics = {k: float(v) for k, v in compute_metrics(test_overall["return"]).items()}
    wf_payload["test"] = {
    ...
    print(
        "  Out-of-sample (test): "
        f"CAGR {test_metrics['cagr']*100:5.2f}% | Sharpe {test_metrics['sharpe']:5.2f} | MaxDD {test_metrics['max_drawdown']*100:6.2f}%"
    )
