...
def compute_monthly_momentum(
    prices: pd.DataFrame, lookback_months: int = 12
) -> pd.DataFrame:
...
def backtest_spy_default(monthly_prices: pd.DataFrame, momentum: pd.DataFrame) -> dict:
    ...
    return_series = pd.Series(
        returns, index=monthly_prices.index[2 : len(monthly_prices)]
    )
    return {
        "label": "spy_default",
        "positions": positions,
...
def main() -> None:
    ...
    prices = pd.read_csv(
        BACKTEST_CACHE
        / f"price_data_{START_DATE}_{END_DATE}_{hashlib.md5(','.join(sorted(['SPY', 'EFA', 'ACWX', 'IEF'])).encode('utf-8')).hexdigest()[:10]}.csv",
        index_col=0,
        parse_dates=True,
    )
