...
class PMTLBacktestEngine:
    ...
    def backtest_sma(self, window: int, fallback) -> pd.Series:
        ...
        # Get MA values at month-end and lag by one period to avoid lookahead
        ma_at_month_end = ma.reindex(monthly_dates, method='ffill')
        ma_at_month_end = ma_at_month_end.shift(1)
        
        # Generate signal on prior information; first period stays in fallback
        signal = (monthly_prices > ma_at_month_end).astype(int).fillna(0)
