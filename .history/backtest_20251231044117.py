...
# Toggle to use a separate CSV with longer history (e.g., pre-2015 data fetched from Yahoo)
USE_LONG_HISTORY_CSV = False
DEFAULT_CSV_PATH = Path("etfs.csv")
LONG_HISTORY_CSV_PATH = Path("etfs_longhistory.csv")
...
def run_backtest() -> None:
    ...
    # Choose which single CSV to load (non-bucketed). Bucketed CSVs still live under CSVs/.
    if USE_LONG_HISTORY_CSV and LONG_HISTORY_CSV_PATH.exists():
        csv_path = LONG_HISTORY_CSV_PATH
        print(f"Using long-history CSV: {csv_path}")
    else:
        if USE_LONG_HISTORY_CSV and not LONG_HISTORY_CSV_PATH.exists():
            print(f"Long-history CSV not found at {LONG_HISTORY_CSV_PATH}, falling back to default etfs.csv")
    ...
    end_date = "2015-12-31"
    force_refresh_data = True  # re-download Yahoo data for this window
        ...
        for lookback in lookback_options:
            for rank_gap_setting in rank_gaps:
                for threshold in thresholds:
                    ...
                    # Only force-refresh once; subsequent scenarios reuse the cache
