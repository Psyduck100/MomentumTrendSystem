...
def backtest_sma_window(
    prices: pd.DataFrame, tbill_df: pd.DataFrame, window: int
) -> pd.Series:
    ...
    tbill_monthly = tbill_df.loc[
        tbill_df.index.to_period("M").isin(monthly_dates.to_period("M")),
        "TB3MS_monthly_ret",
    ]
    ...
    blended_ret = (
        signal_at_month_end.values * monthly_gld_ret.values
...
def test_window_range(
    ...
    for window in range(start_window, end_window + 1, step):
        ...
        print(
            f"  SMA {window:3d}: CAGR {metrics['cagr']:6.2f}%, "
    ...
    for window in range(start_window, end_window + 1, step):
        ...
        print(
            f"  EMA {window:3d}: CAGR {metrics['cagr']:6.2f}%, "
            f"Sharpe {metrics['sharpe']:6.3f}, MaxDD {metrics['max_drawdown']:7.2f}%"
        )
...
if __name__ == "__main__":
    ...
    # Test windows
    results_df, annual_returns_all = test_window_range(
        prices, tbill_df, start_window=100, end_window=200, step=10
    )
