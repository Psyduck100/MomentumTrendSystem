...
def persist_results(
    methodology_name: str, results: List[Dict[str, Any]], csv_path: Path
) -> None:
...
def anchored_walkforward(tickers, bucket_map, start_year=2015, end_year=2025):
    ...
    print(
        f"DEBUG: Will test years from {start_year + initial_train_years} to {end_year}"
    )
    ...
    for test_year in range(start_year + initial_train_years, end_year + 1):
        ...
        print(
            f"    Filter: {best_config['filter_mode']} (band={best_config['filter_band']})"
        )
...
def main():
    ...
    try:
        ...
        print(
            f"DEBUG: anchored_walkforward returned {len(anchored_results) if anchored_results else 0} results"
        )
        ...
        if anchored_results:
            ...
            persist_results(
                "anchored", anchored_results, Path("walkforward_anchored.csv")
            )
