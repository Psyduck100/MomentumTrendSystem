...
EMA 50 Performance:
  ...
  Every 3 days:  3.32% CAGR, 0.835 Sharpe (BREAKS - trading 120x/year)

CRITICAL FINDING:
Every 3-day rebalancing FAILS spectacularly (3.32% CAGR vs 37.96% for weekly).
This reveals severe overfitting/whipsaws at extreme frequencies.

INTERPRETATION:
- Weekly (EMA 50): Optimal - 37.96% CAGR with 3.046 Sharpe, -10.71% MaxDD
...
- 3-day: TOO FREQUENT - Excessive whipsaws destroy returns

RECOMMENDATION FOR LIVE TRADING:
1. BEST: Weekly rebalancing (EMA 50 = 37.96% CAGR, 3.05 Sharpe, -10.71% MaxDD)
2. PRACTICAL: Biweekly (easier to schedule, still 34.12% CAGR)
3. AVOID: Every 3 days (trading costs kill returns; 3.32% CAGR is worse than buy-and-hold)
4. CONSERVATIVE: Monthly (excellent risk metrics but lower returns)

The weekly strategy's 3.046 Sharpe ratio is outstanding for a 20-year backtest.
The collapse at 3-day shows there's a real limit to how frequently you can rebalance.
""")
