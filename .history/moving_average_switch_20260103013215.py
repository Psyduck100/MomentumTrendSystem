"""200-day moving-average switch between a stock ETF and VGIT bonds."""
...
def run_strategy(
    ...
    data = download_prices([stock_ticker, bond_ticker], start, end, cache_dir).sort_index()
    if stock_ticker not in data.columns or bond_ticker not in data.columns:
        raise RuntimeError("Missing ticker data")
    stock = data[stock_ticker].dropna()
    bond = data[bond_ticker].dropna()
    merged = pd.concat([stock, bond], axis=1).dropna()
    merged.columns = ["stock", "bond"]

    daily_returns = merged.pct_change()
    ma = merged["stock"].rolling(ma_length).mean()

    positions = merged["stock"] > ma
    returns = []
    for i in range(1, len(merged)):
        date = merged.index[i]
        in_stock = positions.iloc[i - 1]
        if in_stock:
            ret = daily_returns.iloc[i]["stock"]
        else:
            ret = daily_returns.iloc[i]["bond"]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Moving-average stock/bond switch")
    parser.add_argument("--stock", default="SPY", help="Broad stock ETF (e.g., SPY or VT)")
    parser.add_argument("--bond", default="VGIT", help="Safe bond ETF")
