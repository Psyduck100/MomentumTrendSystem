...
def run_walk_forward_bucket_comparison() -> None:
    ...
    folds = [
        {"train_start": "2015-01-01", "train_end": "2016-12-31", "test_start": "2017-01-01", "test_end": "2018-12-31"},
        ...
        {"train_start": "2015-01-01", "train_end": "2020-12-31", "test_start": "2021-01-01", "test_end": "2022-12-31"},
        {"train_start": "2015-01-01", "train_end": "2021-12-31", "test_start": "2022-01-01", "test_end": "2023-12-31"},
        {"train_start": "2015-01-01", "train_end": "2022-12-31", "test_start": "2023-01-01", "test_end": "2024-12-31"},
    ]
    ...
    for config_name, buckets_to_include in configs.items():
        ...
        for fold_idx, fold in enumerate(folds, 1):
            ...
            print(f"  Fold {fold_idx} ({fold['test_start'][:4]}-{fold['test_end'][:4]}): "
    ...
    for config_name, stats in results.items():
        ...
        print(f"  Sharpe:  median={stats['median_sharpe']:.2f}, mean={stats['mean_sharpe']:.2f} (±{stats['std_sharpe']:.2f})")
        print(f"  CAGR:    median={stats['median_cagr']:.2%}, mean={stats['mean_cagr']:.2%} (±{stats['std_cagr']:.2%})")
        print(f"  MaxDD:   median={stats['median_maxdd']:.2%}, mean={stats['mean_maxdd']:.2%} (±{stats['std_maxdd']:.2%})")
        print(f"  Turnover: median={stats['median_turnover']:.2%}, mean={stats['mean_turnover']:.2%} (±{stats['std_turnover']:.2%})")
    ...
    if len(results) == 2:
        ...
        if with_reits and without_reits:
            ...
            dd_gain = without_reits["median_maxdd"] - with_reits["median_maxdd"]  # Positive = less negative (improvement)
            turnover_change = without_reits["median_turnover"] - with_reits["median_turnover"]
            
            print(f"\nSharpe ratio:   {sharpe_gain:+.2f} ({with_reits['median_sharpe']:.2f} → {without_reits['median_sharpe']:.2f})")
            print(f"CAGR:           {cagr_gain:+.2%} ({with_reits['median_cagr']:.2%} → {without_reits['median_cagr']:.2%})")
            print(f"Max Drawdown:   {dd_gain:+.2%} ({with_reits['median_maxdd']:.2%} → {without_reits['median_maxdd']:.2%})")
            print(f"Turnover:       {turnover_change:+.2%} ({with_reits['median_turnover']:.2%} → {without_reits['median_turnover']:.2%})")
    ...
    if "WITH REITs" in results and "WITHOUT REITs" in results:
        ...
        print(f"{'Test Period':<20} {'WITH REITs Sharpe':<20} {'WITHOUT REITs Sharpe':<20} {'Difference':<15}")
        print("-" * 75)
        
        for wr, wo in zip(with_reits_folds, without_reits_folds):
            diff = wo["sharpe"] - wr["sharpe"]
            print(f"{wr['year']:<20} {wr['sharpe']:<20.2f} {wo['sharpe']:<20.2f} {diff:+.2f}")
