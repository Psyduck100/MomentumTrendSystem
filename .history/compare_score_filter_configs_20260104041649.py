...
def load_risk_free_band() -> pd.Series:
    """Load TB3MS and compute rolling 12M compounded risk-free return."""
    ...
    # monthly risk-free return (approx) from annualized 3M bill rate
    tbill['rf_monthly'] = tbill['TB3MS'] / 100 / 12
    tbill['rf_12m_comp'] = (1 + tbill['rf_monthly']).rolling(12).apply(lambda x: (1 + x).prod() - 1, raw=False)
    return tbill['rf_12m_comp'].dropna()
