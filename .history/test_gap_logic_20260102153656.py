...
def test_rank_gap_logic():
    ...
    print("=" * 70)
    print(
        f"{'Gap':<5} {'Current':<10} {'Rank':<6} {'Expected':<12} {'Actual':<12} {'Status':<8}"
    )
    print("-" * 70)
    ...
    for bucket_gap, current, expected_keep in test_cases:
        ...
        current_rank_str = (
            str(ranked_symbols.index(current)) if current in ranked_symbols else "N/A"
        )
        ...
        print(
            f"{bucket_gap:<5} {current:<10} {current_rank_str:<6} {expected_str:<12} {actual_str:<12} {status:<8}"
        )
    ...
    if failures:
        ...
        for gap, curr, exp, act in failures:
            print(
                f"  Gap={gap}, Current={curr}: expected {'keep' if exp else 'switch'}, got {'keep' if act else 'switch'}"
            )
    else:
