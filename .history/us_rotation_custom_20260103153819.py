...
def main() -> None:
    ...
    benchmarks_by_window: dict[str, dict[str, dict[str, pd.Series] | dict[str, float]]] = {}
    ...
    for window_label, start, end in WINDOWS:
        ...
        for (score_label, score_mode), gap, filter_mode in combos:
            ...
            if benchmark_bundle is None:
                ...
                slice_mask = (monthly_prices.index >= overall.index[0]) & (
                    monthly_prices.index <= overall.index[-1]
                )
                monthly_slice = monthly_prices.loc[slice_mask]
                spy_monthly = monthly_slice["SPY"].pct_change()
                qqq_monthly = monthly_slice["QQQ"].pct_change()
                spy_metrics = compute_metrics(spy_monthly.dropna())
                qqq_metrics = compute_metrics(qqq_monthly.dropna())
                benchmark_bundle = {
