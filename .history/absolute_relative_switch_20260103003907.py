...
2. If the strongest 12-month return is above the absolute threshold, stay in stocks and
   allocate to the stronger symbol (relative momentum).
...
def run_strategy(
    ...
    for idx in range(12, len(monthly) - 1):
        ...
        if best_ret is not None and best_ret > abs_threshold:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Minimum 12M return to stay in stocks (0.0 = >0%)",
    )
    parser.add_argument("--bond-rate", type=float, default=0.04, help="Annualized bond return when defensive")
    parser.add_argument("--output", type=Path, help="Optional CSV path for monthly returns")
...
def main() -> None:
    ...
    print(f"CAGR {metrics['cagr']*100:5.2f}% | Sharpe {metrics['sharpe']:4.2f} | MaxDD {metrics['max_drawdown']*100:6.2f}%")
    print(f"Total Return {(metrics['total_return']*100):6.2f}% | Turnover {turnover:4.2f}")
