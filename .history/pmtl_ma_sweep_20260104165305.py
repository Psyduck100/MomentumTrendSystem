...
def backtest_ema_window(prices: pd.DataFrame, tbill_df: pd.DataFrame, window: int) -> pd.Series:
    ...
    # Generate signal: 1 if price > MA (hold GLD), 0 if price <= MA (hold TB3MS)
    signal = (gld > ma).astype(int)
    ...
    # Get signal at month-end dates
    monthly_signal = signal.loc[monthly_dates]
    ...
    # Blend: signal% GLD + (1-signal%) TB3MS
    blended_ret = (monthly_signal.values * monthly_gld_ret.values +
                   (1 - monthly_signal.values) * tbill_monthly.values)
