...
def build_combo_defs(score_configs: Iterable[ScoreConfig]) -> list[tuple[ScoreConfig, int, str]]:
    return list(product(score_configs, RANK_GAPS, FILTER_MODES))
...
WALK_FORWARD_BLOCKS = [
    {
        "train": ("2002-01-01", "2010-12-31"),
        "test": ("2011-01-01", "2014-12-31"),
    },
    {
        "train": ("2002-01-01", "2014-12-31"),
        "test": ("2015-01-01", "2018-12-31"),
    },
    {
        "train": ("2002-01-01", "2018-12-31"),
        "test": ("2019-01-01", "2022-12-31"),
    },
    {
        "train": ("2002-01-01", "2022-12-31"),
...
def run_walk_forward_analysis() -> dict[str, Any]:
    ...
    def selection_value(snapshot: dict[str, Any]) -> float:
    ...
    for score_config, gap, filter_mode in COMBO_DEFS:
        train_snapshot = evaluate_config_window(
            score_config, gap, filter_mode, TRAIN_START_DATE, TRAIN_END_DATE
        )
        ...
        test_snapshot = evaluate_config_window(
            score_config, gap, filter_mode, TEST_START_DATE, TEST_END_DATE
        )
    ...
    aggregate_candidates.sort(
        key=lambda c: selection_value(c["train"]),
        reverse=True,
    )
    best_config = aggregate_candidates[0]

    wf_payload["candidates"] = [strip_private_keys(c) for c in aggregate_candidates]
    wf_payload["config"] = {
        "score": best_config["score"],
        "score_param": best_config["score_param"],
        "score_param_type": best_config["score_param_type"],
        "rank_gap": best_config["rank_gap"],
        "filter": best_config["filter"],
    }
    wf_payload["train"] = best_config["train"]
    wf_payload["test"] = best_config.get("test")

    train_metrics = best_config["train"]
    objective_desc = (
        f"objective {train_metrics['objective']:.4f}"
    ...
    print(
        ...
        f"CAGR {train_metrics['metrics']['cagr']*100:5.2f}% | Sharpe {train_metrics['metrics']['sharpe']:5.2f} | "
        f"MaxDD {train_metrics['metrics']['max_drawdown']*100:6.2f}% | {objective_desc}"
    )
    ...
    if best_test is None:
        ...
        test_obj = (
            f"objective {best_test['objective']:.4f}"
            if best_test and best_test.get("objective") is not None
            else "objective n/a"
        )
        print(
            "  Out-of-sample (test): "
            f"CAGR {best_test['metrics']['cagr']*100:5.2f}% | Sharpe {best_test['metrics']['sharpe']:5.2f} | "
            f"MaxDD {best_test['metrics']['max_drawdown']*100:6.2f}% | {test_obj}"
        )

    block_entries: list[dict[str, Any]] = []
    selection_counter: Counter[str] = Counter()

    for idx, block in enumerate(WALK_FORWARD_BLOCKS, start=1):
        train_start, train_end = block["train"]
        test_start, test_end = block["test"]
        block_candidates: list[dict[str, Any]] = []

        for score_config, gap, filter_mode in COMBO_DEFS:
            snapshot = evaluate_config_window(
        ...
        winner_config: ScoreConfig = winner["_score_config"]
        test_snapshot = evaluate_config_window(
            winner_config, winner["rank_gap"], winner["filter"], test_start, test_end
        )
        ...
        block_entries.append(
            {
                ...
                "train": winner["train"],
                "test": test_snapshot,
                "train_candidates": [strip_private_keys(c) for c in block_candidates],
            }
        )

        train_obj = winner["train"].get("objective")
        train_obj_desc = (
            f"objective {train_obj:.4f}" if train_obj is not None else "objective n/a"
        )
        if test_snapshot:
            test_obj_desc = (
                f"objective {test_snapshot['objective']:.4f}"
                if test_snapshot.get("objective") is not None
                else "objective n/a"
            )
            test_cagr = test_snapshot["metrics"]["cagr"] * 100
        else:
            test_obj_desc = "objective n/a"
            test_cagr = float("nan")

        print(
            f"  Block {idx}: score={winner['score']} gap={winner['rank_gap']} filter={winner['filter']} | "
            f"train {train_obj_desc} | test CAGR {test_cagr:5.2f}% {test_obj_desc}"
        )

    wf_payload["walk_forward_blocks"] = block_entries
    wf_payload["selection_counts"] = dict(selection_counter)
