...
def run_bucket_drop_walk_forward():
    ...
    print(
        f"Universe: {len(tickers)} tickers across {len(all_buckets)} buckets: {all_buckets}"
    )
    print(
        f"Params: lookback={BEST_PARAMS['lookback']}M, vol_adj={BEST_PARAMS['vol_adjusted']}, gap={BEST_PARAMS['rank_gap']}, threshold={BEST_PARAMS['threshold']}"
    )
    ...
    for cfg_res in configs:
        ...
        print(
            f"  Fold | Period   | Status    | CAGR     | Sharpe | MaxDD    | Turnover"
        )
        print(
            f"  -----|----------|-----------|----------|--------|----------|----------"
        )
        for detail in cfg_res["fold_details"]:
            if detail["status"] == "NO DATA":
                print(
                    f"  {detail['fold']:4d} | {detail['period']} | NO DATA   | -        | -      | -        | -"
                )
            else:
                print(
