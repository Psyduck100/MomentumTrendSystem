...
def generate_configs() -> List[Dict[str, Any]]:
    ...
    gap_configs = [
        ...
        },  # Bonds sticky, equities agile
        {
            ...
            "US_equities": 0,
            "US_small_mid_cap": 0,
        },  # Bonds very sticky, equities aggressive
        {
            "Bonds": 1,
            "Commodities": 2,
            "Emerging_Markets": 1,
...
def walk_forward_test(
    ...
    for i, w in enumerate(windows):
        print(
            f"  Window {i+1}: Train {w['train_start']} to {w['train_end']}, "
    ...
    for window_idx, window in enumerate(windows):
        print(f"\n{'='*100}")
        print(
            f"WINDOW {window_idx + 1}: Train on {window['train_start']} to {window['train_end']}"
        )
        print(f"{'='*100}")
        ...
        print(
            f"  Completed training on {len(train_results)} configs                    "
        )
        ...
        print(
            f"    Filter: {best_config['config']['filter_mode']}@{best_config['config']['filter_band']:.1%}"
        )
        ...
        try:
            ...
            if test_metrics and test_metrics["n_months"] > 6:  # Need at least 6 months
                ...
                print(
                    f"  Insufficient test data (only {test_metrics['n_months'] if test_metrics else 0} months)"
                )
        except Exception as e:
    ...
    if len(df_results) > 0:
        ...
        sharpe_degradation = (
            df_results["train_sharpe"].mean() - df_results["test_sharpe"].mean()
        )
        print(f"  Sharpe degradation: {sharpe_degradation:.3f}")
        print(
            f"  CAGR degradation: {(df_results['train_cagr'].mean() - df_results['test_cagr'].mean()):.2%}"
        )
