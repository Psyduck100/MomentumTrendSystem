...
def backtest_gld_only(prices: pd.DataFrame) -> dict:
    """Benchmark: hold GLD always. Returns full return series."""
...
def main():
    ...
    # Compute annual returns for each window
    print("\n" + "="*100)
    print("ANNUAL RETURNS BY WINDOW")
    print("="*100)
    
    gld_result = backtest_gld_only(prices)
    gld_monthly = gld_result["monthly_returns"]
    gld_annual = gld_monthly.resample("YE").apply(lambda x: (1 + x).prod() - 1)
    
    annual_data = []
    for window in range(100, 201, 10):
        sma_result = backtest_sma_window(prices, window)
        sma_monthly = sma_result["monthly_returns"]
        sma_annual = sma_monthly.resample("YE").apply(lambda x: (1 + x).prod() - 1)
        
        ema_result = backtest_ema_window(prices, window)
        ema_monthly = ema_result["monthly_returns"]
        ema_annual = ema_monthly.resample("YE").apply(lambda x: (1 + x).prod() - 1)
        
        for year, gld_ret in gld_annual.items():
            year_str = str(year.year)
            sma_ret = sma_annual.get(year, None)
            ema_ret = ema_annual.get(year, None)
            
            annual_data.append({
                "year": year_str,
                "window": window,
                "sma_annual_ret": sma_ret,
                "ema_annual_ret": ema_ret,
                "gld_annual_ret": gld_ret,
                "sma_vs_gld": sma_ret - gld_ret if sma_ret else None,
                "ema_vs_gld": ema_ret - gld_ret if ema_ret else None,
            })
    
    annual_df = pd.DataFrame(annual_data)
    annual_csv_path = Path("pmtl_ma_sweep_annual_returns.csv")
    annual_df.to_csv(annual_csv_path, index=False, float_format="%.4f")
    print(f"\nAnnual returns by window exported to {annual_csv_path}")
