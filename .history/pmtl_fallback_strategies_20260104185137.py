...
class SpyThenIefFallback(FallbackStrategy):
    """Fallback: If SPY 12m return > 0, use SPY; else use IEF."""

    def __init__(self, start_date: str, end_date: str):
        super().__init__(start_date, end_date)
        self.spy = YFinanceFallback(start_date, end_date, "SPY")
        self.ief = YFinanceFallback(start_date, end_date, "IEF")

    def get_monthly_returns(self, monthly_dates: pd.DatetimeIndex) -> pd.Series:
        spy_prices = self.spy._data.resample("ME").last()
        spy_12m_ret = spy_prices.pct_change(periods=12)
        spy_signal = (spy_12m_ret > 0).shift(1)
        spy_ret = self.spy.get_monthly_returns(monthly_dates)
        ief_ret = self.ief.get_monthly_returns(monthly_dates)
        # Align all
        df = pd.concat([spy_ret, ief_ret, spy_signal], axis=1)
        df.columns = ["spy", "ief", "signal"]
        df = df.dropna()
        # Debug: print which fallback is chosen each month
        chosen = df["signal"].map(lambda x: "SPY" if x else "IEF")
        print("[SpyThenIefFallback] Fallback asset by month:")
        for date, asset in chosen.items():
            print(f"  {date.strftime('%Y-%m-%d')}: {asset}")
        blended = df["signal"] * df["spy"] + (1 - df["signal"]) * df["ief"]
        return blended.reindex(monthly_dates, fill_value=0.0).rename("spy_then_ief_returns")
