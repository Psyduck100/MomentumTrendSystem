...
def _summarize(df: pd.DataFrame, top_n: int) -> None:
    ...
    print(
        df.sort_values("sharpe", ascending=False).head(top_n)[
            [
                "score_mode",
                "filter_mode",
                "filter_band",
    ...
    print("\nTop configs by CAGR")
    print(
        df.sort_values("cagr", ascending=False).head(top_n)[
            [
                "score_mode",
                "filter_mode",
                "filter_band",
...
def main() -> None:
    ...
    parser.add_argument(
        "--top", type=int, default=10, help="Rows to display in the console"
    )
