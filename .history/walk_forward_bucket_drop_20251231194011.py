...
def evaluate_buckets(
    ...
    for fold_idx, fold in enumerate(FOLDS, 1):
        ...
        fold_details.append({
            ...
            "maxdd": metrics["max_drawdown"],
            "turnover": turnover,
            "n_tickers_used": len(set().union(*[set(p) for p in backtest_data["overall_positions"].values()]) if backtest_data["overall_positions"] else set()),
            "status": "OK"
        })
...
def run_bucket_drop_walk_forward():
    ...
    print(f"Walk-forward bucket drop test (targeting CAGR, preserving diversity)")
    ...
    print(f"Folds: {len(FOLDS)} (expanding window, 2y test), covering 2015-2024")
    print("=" * 100)
    ...
    for idx, cfg_res in enumerate(configs, 1):
        print(
            f"{idx:2d}. {cfg_res['name']:<20} | Sharpe med={cfg_res['median_sharpe']:.2f}, CAGR med={cfg_res['median_cagr']:.2%}, "
    ...
    if baseline:
        for cfg_res in configs:
            ...
            print(
                f"{cfg_res['name']:<20} | Sharpe {sharpe_gain:+.2f}, CAGR {cagr_gain:+.2%}, MaxDD {dd_gain:+.2%}, Turnover {turnover_change:+.2%}"
            )
