...
def main():
    ...
    print(
        f"   Windows: {ACTIVE_CONFIG.windows.start_window}-{ACTIVE_CONFIG.windows.end_window} "
        f"(step {ACTIVE_CONFIG.windows.step})"
    )
    print(
        f"   Date Range: {ACTIVE_CONFIG.dates.start_date} to {ACTIVE_CONFIG.dates.end_date}"
    )
    ...
    # Optional trailing return strategies (absolute momentum)
    if ACTIVE_CONFIG.trailing_return_periods:
        periods_per_year_map = {"ME": 12, "W": 52, "2W": 26, "3D": 122, "D": 252}
        periods_per_year = periods_per_year_map.get(ACTIVE_CONFIG.dates.frequency, 12)

        for period in ACTIVE_CONFIG.trailing_return_periods:
            ret_series = engine.backtest_trailing_return(periods=period, fallback=fallback)
            metrics = compute_metrics(ret_series, periods_per_year)
            ...
            annual = ret_series.groupby(ret_series.index.year).apply(lambda x: (1 + x).prod() - 1)
            annual_returns_all[f'RET_{period}m'] = annual
            monthly_returns_all[f'RET_{period}m'] = ret_series

            print(f"  RET{period:2d}m: CAGR {metrics['cagr']:6.2%}, Sharpe {metrics['sharpe']:6.3f}, MaxDD {metrics['max_drawdown']:7.2%}")
