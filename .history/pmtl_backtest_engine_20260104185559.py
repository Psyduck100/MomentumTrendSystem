...
class PMTLBacktestEngine:
    ...
    def backtest_sma(self, window: int, fallback) -> pd.Series:
        ...
        total = len(df)
        print(f"[PMTLBacktestEngine] SMA window {window}: fallback used {fallback_count}/{total} periods ({fallback_count/total:.1%}), main asset held {main_count}/{total} periods ({main_count/total:.1%})")
