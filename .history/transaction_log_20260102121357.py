...
def run_transaction_log() -> None:
    ...
    print("Config: vol_adj=False, score=rw_3_6_9_12, thr=none, gap=2, abs=rw_3_6_9_12@0.00%")
    ...
    if len(dates) != len(overall_positions):
        print(f"Warning: Date mismatch ({len(dates)} dates vs {len(overall_positions)} positions)")
    ...
    for bucket in buckets:
        ...
        print(f"\n{bucket}:")
        print("-" * 100)
        print(f"{'Date':<12} {'Action':<10} {'Symbol':<8} {'From':<8} {'Description':<40}")
        print("-" * 100)
    ...
    for bucket in buckets:
        bucket_txns = [t for t in transaction_list if t["bucket"] == bucket and t["action"] == "ROTATE"]
        buys = [t for t in transaction_list if t["bucket"] == bucket and t["action"] == "BUY"]
        print(f"{bucket:<20} | Initial Buys: {len(buys):<3} | Rotations: {len(bucket_txns):<3} | Total Activity: {len(buys) + len(bucket_txns)}")
    ...
    for bucket, df_returns in bucket_returns_dict.items():
        ...
        print(f"{bucket:<20} | CAGR: {cagr:>7.2%} | Sharpe: {sharpe:>6.2f} | MaxDD: {max_dd:>8.2%}")
    ...
    for month_idx, (date, positions) in enumerate(zip(dates, overall_positions)):
        ...
        if month_holdings != current_holdings:
            ...
            for bucket in sorted(month_holdings.keys()):
                ...
                change = " [NEW]" if prev == "N/A" else f" [was {prev}]" if prev != symbol else ""
