...
class StrategyConfig:
    ...
    score_mode: str = (
        SCORE_MODE_12M_MINUS_1M  # default live: 12M return minus 1M return
    )
    vol_adjusted: bool = False
    market_filter: bool = False
    market_ticker: str = "SPY"
    defensive_bucket: str = "Bonds"
    rank_gap_threshold: dict[str, int] = field(
        default_factory=lambda: {
            "US_equities": 2,
            "US_small_mid_cap": 1,
            # All other buckets default to 0 (not listed)
        }
    )
    abs_filter_mode: str = "none"  # none, ret_6m, ret_12m, ret_and, ret_blend
    abs_filter_band: float = 0.0  # e.g., 0.01 = require >1%
    abs_filter_cash_annual: float = 0.04  # cash/T-bill rate when filtered
