...
def build_targets(
    ...
    available_buckets = set(grouped.keys())
    market_filter_enabled = (
        cfg.market_filter
        and not market_is_positive
        and cfg.defensive_bucket in available_buckets
    )
    # If the defensive bucket is not present (e.g., Bonds excluded), skip the filter to avoid empty portfolios.
    if cfg.market_filter and not market_is_positive and cfg.defensive_bucket not in available_buckets:
        market_is_positive = True
    ...
    for bucket, scores in grouped.items():
        ...
        if market_filter_enabled and bucket != cfg.defensive_bucket:
