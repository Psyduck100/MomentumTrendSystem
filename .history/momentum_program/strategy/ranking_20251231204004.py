...
def build_targets(
    ...
    # Disable market filter if the defensive bucket is excluded; otherwise filter can blank the portfolio.
    market_filter_enabled = cfg.market_filter and cfg.defensive_bucket not in excluded_buckets
    ...
    for bucket, scores in grouped.items():
        ...
        if market_filter_enabled and not market_is_positive and bucket != cfg.defensive_bucket:
        ...
        if leader and current and cfg.rank_gap_threshold > 0 and current in ranked_symbols:
