...
def build_targets(
    ...
    momentum_scores = rank_momentum(
        price_series,
        vol_adjusted=cfg.vol_adjusted,
        score_mode=getattr(cfg, "score_mode", "rw_3_6_9_12"),
        score_param=getattr(cfg, "score_param", None),
    )
    ...
    for bucket, scores in grouped.items():
        ...
        # Pick leader with rank-gap turnover control (store symbol only)
        leader_symbol = top_scores[0].symbol if top_scores else None
        ...
        if isinstance(cfg.rank_gap_threshold, dict):
            bucket_gap = cfg.rank_gap_threshold.get(bucket, 0)
        else:
            bucket_gap = cfg.rank_gap_threshold

        if leader_symbol and current and bucket_gap > 0 and current in ranked_symbols:
            leader_rank = ranked_symbols.index(leader_symbol)
            current_rank = ranked_symbols.index(current)
            if leader_rank > current_rank - bucket_gap:
                leader_symbol = current
        # If prior pick isn't in the current ranking (e.g., dropped from universe), don't keep it.
        if leader_symbol and leader_symbol not in ranked_symbols:
            leader_symbol = None

        bucket_selection[bucket] = leader_symbol
        if leader_symbol:
            match = next((s for s in sorted_scores if s.symbol == leader_symbol), None)
            if match:
