...
def backtest_momentum(
    ...
    for i in range(max(lookback_long, lookback_short), len(momentum)):
        ...
        if market_filter and market_ticker in valid_tickers:
            market_ret_12m = monthly.loc[date, market_ticker]
            market_ret_12m_start = monthly.iloc[max(0, i - 12)][market_ticker]
            if market_ret_12m_start > 0:
                market_momentum = (market_ret_12m - market_ret_12m_start) / market_ret_12m_start
                market_is_positive = market_momentum > 0
