...
def compute_metrics(returns: pd.Series, periods_per_year: int = 12) -> dict:
    """Compute CAGR, volatility, Sharpe, max drawdown, Sortino for a return series.
    
    Args:
        returns: Series of period returns
        periods_per_year: Number of periods per year (12=monthly, 52=weekly, 26=biweekly)
    """
    if len(returns) == 0:
        return {
            "cagr": 0.0,
    ...
    downside_vol = (
        downside_returns.std() * np.sqrt(periods_per_year) if len(downside_returns) > 0 else 0.0
    )
