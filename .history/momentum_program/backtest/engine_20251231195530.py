...
def backtest_momentum(
    ...
    # Relax threshold to 50% to improve coverage for older periods
    ...
    if vol_adjusted:
        print(
            f"Applying volatility adjustment (over {lookback_long}M lookback period)..."
        )
        # Compute rolling volatility over same lookback_long period (annualized)
        # This ensures we normalize momentum by the volatility experienced during the same period
        momentum_vol = monthly_returns.rolling(lookback_long).std() * np.sqrt(12)
        # Divide momentum by volatility to get risk-adjusted score (replace inf/nan with 0)
