...
def backtest_momentum(
    ...
    Args:
        ...
        slippage_bps: Trading slippage in basis points (default 5 bps per trade).
        expense_ratio: Annual expense ratio drag (default 0.15% annualized).
    ...
    for i in range(max(lookback_long, lookback_short), len(momentum)):
        ...
        slippage_cost = (trades / len(selected_symbols)) * (slippage_bps / 10000) if len(selected_symbols) > 0 else 0.0
        ...
        for bucket in buckets:
            ...
            if bucket_symbols:
                ...
                if isinstance(bucket_ret, pd.Series):
                    bucket_ret = bucket_ret.dropna().mean()
                else:
                    bucket_ret = np.nanmean(bucket_ret)
