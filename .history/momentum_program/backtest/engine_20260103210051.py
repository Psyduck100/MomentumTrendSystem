        ...
        for bucket in buckets:
            ...
            if filter_pass:
                ...
                if defensive_symbol and defensive_symbol in monthly_returns.columns:
                    guard_symbol = defensive_guard.get("symbol") if defensive_guard else None
                    guard_symbol = guard_symbol or defensive_symbol
                    guard_pass = True
                    if defensive_guard:
                        guard_price_series = monthly[guard_symbol] if guard_symbol in monthly.columns else None
                        ...
                        if guard_price_series is not None:
                            idx_pos = monthly.index.get_loc(date)
                            if isinstance(idx_pos, slice):
                                idx_pos = idx_pos.start
                            if isinstance(idx_pos, (int, np.integer)) and idx_pos >= lookback_days:
                                window = guard_price_series.iloc[idx_pos - lookback_days : idx_pos]
                                if not window.empty and not pd.isna(window).all():
                                    ma = window.mean()
                                    price = guard_price_series.iloc[idx_pos]
