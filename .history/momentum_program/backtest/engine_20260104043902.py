...
def _recency_weights(alpha: float | None) -> dict[int, float]:
    """Return normalized weights for the recency-weighted momentum family."""
...
def backtest_momentum(
    ...
    abs_filter_band_series: pd.Series | None = None,  # optional date-indexed band (e.g., rolling risk-free)
    abs_filter_cash_annual: float = 0.04,  # cash/T-bill proxy when filtered
    defensive_symbol: str | None = None,
    defensive_guard: dict | None = None,
) -> dict:
    """
    ...
    Args:
        ...
        score_mode: Momentum formula to use. Options:
            ...
            - SCORE_MODE_RW_ALPHA: Continuous recency weights (alpha controls emphasis on shorter horizons).
            - SCORE_MODE_BLEND_LAMBDA: Two-horizon blend with lambda controlling the 6M-vs-12M mix.
        score_param: Optional parameter passed to the selected score_mode (alpha for RW, lambda for blend).
        abs_filter_mode: Absolute momentum filter applied per bucket after ranking. If filter fails, bucket goes to cash.
            ...
            - Any SCORE_MODE_* value will reuse the corresponding momentum formula (and score_param when applicable).
        ...
        defensive_symbol: Optional ticker to use instead of cash when the absolute filter fails
            (e.g., rotate into a bond ETF such as IEF). The ticker must be included in `tickers` so
            its monthly returns are available. Falls back to cash if the symbol is missing/NaN.
        defensive_guard: Optional dict describing when the defensive symbol is allowed. Supported keys:
            - symbol: ticker to guard (defaults to defensive_symbol if omitted)
            - lookback_days: trading days for the moving average (default 100)
            - threshold: require price >= (1 + threshold) * moving average (default 0)
    ...
    cache_file = (
        cache_dir / f"price_data_{start_date}_{end_date}_{ticker_fingerprint}.csv"
    )
    ...
    def _download_prices(symbols: list[str]) -> pd.DataFrame:
        ...
        for i in range(0, len(symbols), chunk_size):
            ...
            if isinstance(df_chunk.columns, pd.MultiIndex):
                ...
                ticker_labels = df_chunk.columns.get_level_values(0).unique()
                ...
                df_chunk = (
                    pd.concat(selected_cols, axis=1)
                    if selected_cols
                    else pd.DataFrame(index=df_chunk.index)
                )
    ...
    while True:
        ...
        print("Cache produced insufficient coverage; refreshing data...")
        if cache_file.exists():
            cache_file.unlink(missing_ok=True)
        data = _download_prices(tickers)
        if data.empty:
            print("Download returned no data after cache refresh; aborting backtest.")
            return _empty_result()
        print(f"Saving price data to cache: {cache_file}")
        data.to_csv(cache_file)
        refreshed_cache = True
    print(f"Using {len(valid_tickers)} tickers with sufficient history")
    daily_price_data = data.copy()
    ...
    param_note = "" if score_param is None else f" (param={score_param})"
    print(f"Computing momentum (mode={score_mode}{param_note})...")
    ...
    if score_mode == SCORE_MODE_12M_MINUS_1M:
    ...
    elif score_mode == SCORE_MODE_12M:
        momentum = ret_12
        min_lookback_months = 12
        ...
        lam = 0.5 if score_param is None else float(score_param)
        ...
        for bucket in buckets:
            ...
            if abs_filter_mode != "none":
                ...
                if abs_filter_mode == "ret_6m" and ret_6 is not None:
                    ...
                    abs_filter_mode == "ret_and"
                    and ret_6 is not None
                    and ret_12 is not None
                ):
                    score = min(ret_6, ret_12)
                elif (
                    abs_filter_mode == "ret_blend"
                    and ret_6 is not None
                    and ret_12 is not None
                ):
                    score = 0.6 * ret_12 + 0.5 * ret_6
                elif abs_filter_mode == SCORE_MODE_RW_3_6_9_12 and all(
                    x is not None for x in [ret_3, ret_6, ret_9, ret_12]
                ):
                    score = 0.4 * ret_3 + 0.2 * ret_6 + 0.2 * ret_9 + 0.2 * ret_12
                elif abs_filter_mode == SCORE_MODE_RW_ALPHA and all(
                    x is not None for x in [ret_3, ret_6, ret_9, ret_12]
                ):
                    weights = _recency_weights(score_param)
                    score = (
                    ...
                    abs_filter_mode == SCORE_MODE_12M_MINUS_1M
                    and ret_12 is not None
                    and ret_1 is not None
                ):
                    score = ret_12 - ret_1
                elif (
                    abs_filter_mode == SCORE_MODE_BLEND_6_12
                    and ret_6 is not None
                    and ret_12 is not None
                ):
                    score = 0.5 * ret_6 + 0.5 * ret_12
                elif (
                    abs_filter_mode == SCORE_MODE_BLEND_LAMBDA
                    and ret_6 is not None
                    and ret_12 is not None
                ):
                    lam = 0.5 if score_param is None else float(score_param)
                    lam = min(max(lam, 0.0), 1.0)
                    score = lam * ret_6 + (1 - lam) * ret_12

            filter_pass = True
            if score is not None:
                band_value = abs_filter_band
                if abs_filter_band_series is not None:
                    val = abs_filter_band_series.get(next_date, abs_filter_band)
                    band_value = abs_filter_band if pd.isna(val) else float(val)
                filter_pass = score > band_value
            ...
            if filter_pass:
                ...
                if isinstance(bucket_ret, pd.Series):
                    ...
                    bucket_ret_gross = (
                        float(bucket_ret) if not pd.isna(bucket_ret) else 0.0
                    )
                ...
                # Go to defensive asset (if provided) or cash/T-bills for this bucket
                bucket_slippage = 0.0
                bucket_expense = 0.0
                bucket_ret_gross = cash_ret_monthly
                bucket_ret_net = cash_ret_monthly
                
                # Special handling for TB3MS (T-bills via risk-free rate)
                if defensive_symbol == "TB3MS" and abs_filter_band_series is not None:
                    if next_date in abs_filter_band_series.index:
                        # abs_filter_band_series already contains 12M compounded risk-free return
                        # Convert back to monthly return for this month's allocation
                        annual_rf_12m = abs_filter_band_series.loc[next_date]
                        if pd.notna(annual_rf_12m) and annual_rf_12m > 0:
                            # Approximate monthly return from annualized compounded return
                            monthly_rf = (1 + annual_rf_12m) ** (1/12) - 1
                            bucket_ret_gross = bucket_ret_net = float(monthly_rf)
                        else:
                            bucket_ret_gross = bucket_ret_net = cash_ret_monthly
                    else:
                        bucket_ret_gross = bucket_ret_net = cash_ret_monthly
                elif defensive_symbol and defensive_symbol in monthly_returns.columns:
                    guard_symbol = defensive_symbol
                    guard_pass = True
                    if defensive_guard:
                        guard_symbol = defensive_guard.get("symbol", defensive_symbol)
                        guard_price_series = (
                            daily_price_data[guard_symbol]
                            if guard_symbol in daily_price_data.columns
                            else None
                        )
                        lookback_days = defensive_guard.get("lookback_days", 100)
                        threshold = defensive_guard.get("threshold", 0.0)
                        guard_pass = False
                        if guard_price_series is not None:
                            history = guard_price_series.dropna()
                            if not history.empty:
                                aligned = history.loc[:date]
                                if len(aligned) >= lookback_days:
                                    window = aligned.iloc[-lookback_days:]
                                    price = window.iloc[-1]
                                    ma = window.mean()
                                    if pd.notna(price) and pd.notna(ma) and ma != 0:
                                        guard_pass = price >= (1 + threshold) * ma
                    if guard_pass:
                        defensive_ret = monthly_returns.loc[next_date, defensive_symbol]
                        if pd.notna(defensive_ret):
                            bucket_ret_gross = bucket_ret_net = float(defensive_ret)
