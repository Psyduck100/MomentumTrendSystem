...
def backtest_momentum(
    ...
    use_cache = cache_file.exists() and not force_refresh
    ...
    if use_cache:
        ...
        if force_refresh and cache_file.exists():
            print(f"Force refresh: ignoring cache at {cache_file}")
        ...
        for i in range(0, len(tickers), chunk_size):
            ...
            raw_chunk = yf.download(chunk, start=start_date, end=end_date, progress=False)
            normalized = _normalize_price_frame(raw_chunk)
        ...
        # Save to cache (even if empty to avoid repeated downloads under rate limits)
