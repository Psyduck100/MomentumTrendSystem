...
def run_backtest() -> None:
    ...
    # Generate all combinations of gap values (0-3) for each bucket
    all_buckets = sorted(list(set(bucket_map.values())))
    from itertools import product
    gap_values = [0, 1, 2, 3]
    gap_combinations = [dict(zip(all_buckets, combo)) for combo in product(gap_values, repeat=len(all_buckets))]
    
    # Limit to reasonable number of combinations (too many = too slow)
    # Start with uniform and some hand-picked per-bucket configs
    rank_gaps = [
        0,  # uniform 0 (no gap)
        1,  # uniform 1
    ...
    # Add all per-bucket combinations for stress testing
    rank_gaps.extend(gap_combinations)
    ...
    # Write gap stress test results (show optimal gap per bucket)
    ...
    with gap_stress_path.open("w") as f:
        ...
        stress_scenarios = [s for s in scenarios if s['start'] == '2015-01-01' and s['end'] == '2025-12-31' and s['score_mode'] == SCORE_MODE_RW_3_6_9_12]
        ...
        for abs_mode, abs_band in abs_filters:
            ...
            filter_scenarios = [s for s in stress_scenarios if s['abs_filter'] == abs_mode and s['abs_band'] == abs_band]
            ...
            for s in sorted(filter_scenarios, key=lambda x: x['overall']['sharpe'], reverse=True)[:20]:
                gap_val = s['rank_gap']
                if isinstance(gap_val, dict):
                    gap_txt = ", ".join([f"{k}:{v}" for k, v in sorted(gap_val.items())])
                else:
                    gap_txt = f"Uniform={gap_val}"
                
                f.write(f"{gap_txt:<50} | {s['overall']['sharpe']:14.2f} | {s['turnover']:8.2%} |\n")
            
            # Per-bucket analysis
            f.write(f"\n{'BUCKET':<25} | {'Best Gap':<20} | CAGR | Sharpe | MaxDD | Turnover\n")
            f.write("-" * 120 + "\n")
            ...
            for bucket in all_bucket_names:
                ...
                for s in filter_scenarios:
                    if bucket in s['buckets']:
                        bucket_sharpe = s['buckets'][bucket]['sharpe']
                        if bucket_sharpe > best_sharpe:
                            best_sharpe = bucket_sharpe
                            gap_val = s['rank_gap']
                            if isinstance(gap_val, dict):
                                best_gap = gap_val.get(bucket, 0)
                            else:
                                best_gap = gap_val
                            best_metrics = s['buckets'][bucket]
                
                if best_metrics:
                    f.write(f"{bucket:<25} | Gap={best_gap:<18} | {best_metrics['cagr']:4.2%} | {best_metrics['sharpe']:6.2f} | {best_metrics['max_drawdown']:6.2%} | \n")
