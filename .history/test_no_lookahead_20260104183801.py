...
def test_sma_signal_is_lagged_one_period():
    ...
    dates_raw = pd.date_range("2020-01-03", periods=4, freq="W-FRI")
    prices = pd.Series([100.0, 110.0, 90.0, 120.0], index=dates_raw)
    ...
    # With warmup + shift(1), the first tradable bar is 2020-01-12.
    expected_index = pd.date_range("2020-01-12", periods=3, freq="W-SUN")
    expected = pd.Series([0.0, -0.1818181818, 0.0], index=expected_index, name="SMA_2")
...
def test_ema_signal_is_lagged_one_period():
    ...
    # With warmup + shift(1), the first tradable bar is 2020-01-12.
    expected_index = pd.date_range("2020-01-12", periods=3, freq="W-SUN")
    expected = pd.Series([0.0, -0.1818181818, 0.0], index=expected_index, name="EMA_2")
