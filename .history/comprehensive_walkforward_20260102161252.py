...
# Configuration space
SCORE_MODES = ['rw_3_6_9_12', '12m_minus_1m', 'blend_6_12']
FILTER_MODES = [
    {'mode': 'none', 'band': None},
...
GAP_CONFIGS = [
    ...
    {'US_equities': 0, 'Intl_equities': 0, 'US_bonds': 0, 'Intl_bonds': 0, 'Commodities': 0, 'Cash': 0},  # All agile
    {'US_equities': 1, 'Intl_equities': 1, 'US_bonds': 1, 'Intl_bonds': 1, 'Commodities': 1, 'Cash': 1},  # All moderate
    {'US_equities': 2, 'Intl_equities': 2, 'US_bonds': 3, 'Intl_bonds': 3, 'Commodities': 2, 'Cash': 2},  # Sticky bonds
    {'US_equities': 1, 'Intl_equities': 1, 'US_bonds': 2, 'Intl_bonds': 2, 'Commodities': 1, 'Cash': 1},  # Moderate bonds
    {'US_equities': 0, 'Intl_equities': 0, 'US_bonds': 3, 'Intl_bonds': 3, 'Commodities': 1, 'Cash': 1},  # Agile equities, sticky bonds
    {'US_equities': 3, 'Intl_equities': 3, 'US_bonds': 1, 'Intl_bonds': 1, 'Commodities': 2, 'Cash': 2},  # Sticky equities, agile bonds
    {'US_equities': 0, 'Intl_equities': 1, 'US_bonds': 2, 'Intl_bonds': 2, 'Commodities': 1, 'Cash': 0},  # Mixed agility
]
...
def generate_configs():
    ...
    for score_mode in SCORE_MODES:
        for filter_spec in FILTER_MODES:
            for rank_gap in GAP_CONFIGS:
                configs.append({
                    'score_mode': score_mode,
                    'filter_mode': filter_spec['mode'],
                    'filter_band': filter_spec['band'],
                    'rank_gap': rank_gap
                })
    return configs


def run_backtest_config(config, start_date, end_date):
    """Run backtest for a specific configuration and date range."""
    try:
        # Load data
        ...
        # Resample to monthly
        monthly = resample_to_monthly(prices)
...
def anchored_walkforward(start_year=2015, end_year=2025):
    """
    ...
    for test_year in range(start_year + initial_train_years, end_year + 1):
        ...
        for i, config in enumerate(all_configs):
            ...
            train_metrics = run_backtest_config(config, train_start, train_end)
        ...
        print(f"    Score: {best_config['score_mode']}")
        print(f"    Filter: {best_config['filter_mode']}" + 
              (f"@{best_config['filter_band']*100:.1f}%" if best_config['filter_band'] else ""))
        print(f"    Gap: {best_config['rank_gap']}")
        
        # Test: apply to test year
        print(f"\n  Testing on {test_year}...")
        test_metrics = run_backtest_config(best_config, test_start, test_end)
...
def rolling_walkforward(train_years=5, start_year=2015, end_year=2025):
    """
...
def print_summary(test_name, results):
    ...
    for config_str in configs:
        ...
        filter_mode = config['filter_mode']
        if config['filter_band']:
            filter_mode += f"@{config['filter_band']*100:.0f}%"
...
def main():
    ...
    anchored_results = anchored_walkforward(start_year=2015, end_year=2025)
    
    # Save results
    df_anchored = pd.DataFrame([{k: v for k, v in r.items() if k != 'test_returns'} 
                                for r in anchored_results])
    ...
    rolling5_results = rolling_walkforward(train_years=5, start_year=2015, end_year=2025)
    
    df_rolling5 = pd.DataFrame([{k: v for k, v in r.items() if k != 'test_returns'} 
                                for r in rolling5_results])
    ...
    rolling4_results = rolling_walkforward(train_years=4, start_year=2015, end_year=2025)
    
    df_rolling4 = pd.DataFrame([{k: v for k, v in r.items() if k != 'test_returns'} 
                                for r in rolling4_results])
    ...
    regime_results = regime_split_test()
    
    df_regime = pd.DataFrame([{k: v for k, v in r.items() if k != 'test_returns'} 
    ...
    for name, results in [
        ...
        if results:
            stitched = stitch_equity_curve(results)
            print(f"{name:<30} {stitched['cagr']*100:>7.2f}% {stitched['sharpe']:>7.2f} "
