...
def run_strategy(
    ...
    if defensive_ticker:
        if defensive_ticker not in data.columns:
            raise RuntimeError(f"Missing defensive ticker {defensive_ticker}")
        defensive_returns = data[defensive_ticker].pct_change().reindex(stock.index).fillna(0)
