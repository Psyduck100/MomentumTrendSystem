...
import PMTL.PMTL as PMTL
import USEQ.USEQ as USEQ
...
def build_useq_recommendations(
    ...
    *,
    ...
    monthly_returns = USEQ.compute_lookback_returns(monthly_prices, config.lookback_months)
    ...
    for asof in monthly_prices.index:
        ...
        rank_table = USEQ.build_rank_table(monthly_returns, scores, asof=asof, abs_months=12)
...
def main():
    ...
    combined_returns = gld_returns.where(is_bull, useq_returns)

    gld_bull_returns = gld_returns.where(is_bull, 0.0)
    useq_nonbull_returns = useq_returns.where(~is_bull, 0.0)

    print("GLD Bull Sleeve:", perf_metrics(gld_bull_returns))
    print("USEQ Non-Bull Sleeve:", perf_metrics(useq_nonbull_returns))
    print("Combined Layered:", perf_metrics(combined_returns))
