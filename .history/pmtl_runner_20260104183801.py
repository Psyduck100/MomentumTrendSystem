...
def main():
    ...
    if ACTIVE_CONFIG.trailing_return_periods:
        ...
        for period in ACTIVE_CONFIG.trailing_return_periods:
            ret_series = engine.backtest_trailing_return(
                periods=period, fallback=fallback
            )
            ...
            annual = ret_series.groupby(ret_series.index.year).apply(
                lambda x: (1 + x).prod() - 1
            )
            ...
            print(
                f"  RET{period:2d}m: CAGR {metrics['cagr']:6.2%}, Sharpe {metrics['sharpe']:6.3f}, MaxDD {metrics['max_drawdown']:7.2%}"
            )
