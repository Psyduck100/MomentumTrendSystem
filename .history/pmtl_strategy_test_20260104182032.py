"""PMTL Strategy: GLD with 50/100/200-day trading MA filters vs 12M return filter.

Compare seven strategies:
1. GLD 50-day SMA filter: hold GLD if above 50 trading-day SMA, else IEF
2. GLD 100-day SMA filter: hold GLD if above 100 trading-day SMA, else IEF
3. GLD 100-day EMA filter: hold GLD if above 100 trading-day EMA, else IEF
4. GLD 200-day SMA filter: hold GLD if above 200 trading-day SMA, else IEF
5. GLD 6M return filter: hold GLD if 6M > 0, else IEF
6. GLD 12M return filter: hold GLD if 12M > 0, else IEF
7. GLD benchmark: hold GLD always
"""
...
class FilterType(Enum):
    ...
    Note: yfinance daily data contains only trading days (no weekends/holidays).
    So rolling(window=100) on daily data = 100 trading days of SMA.
    No calendar conversion needed.
    """

    MA_50 = ("50TMA", 50)  # 50 trading days
    MA_100 = ("100TMA", 100)  # 100 trading days
    MA_200 = ("200TMA", 200)  # 200 trading days
...
def download_prices(
    tickers: list[str], end_date: str = END_DATE, start_date: str = START_DATE
) -> pd.DataFrame:
...
def backtest_trailing_return(prices: pd.DataFrame, months: int) -> dict:
    ...
    for i in range(1, len(monthly)):
        if i >= months:
            gld_return = (
                monthly.iloc[i]["GLD"] - monthly.iloc[i - months]["GLD"]
            ) / monthly.iloc[i - months]["GLD"]
        else:
            gld_return = 0
...
def run_four_year_chunks() -> None:
    ...
    for start_year in start_years:
        ...
        print("\n" + "=" * 140)
        print(
            f"GLD FILTER COMPARISON {start_year}-{end_year} ({ma50.index[0].strftime('%Y-%m-%d')} to {ma50.index[-1].strftime('%Y-%m-%d')})"
        )
        print("=" * 140)
...
def run_scenario(end_label: str, end_date: str) -> None:
    ...
    if all(
        ...
        print("\n" + "=" * 160)
        print(
            f"GLD FILTER COMPARISON {end_label} ({ma50_ret.index[0].strftime('%Y-%m-%d')} to {ma50_ret.index[-1].strftime('%Y-%m-%d')})"
        )
        print(f"Trading days: 50~{cal_50}cal, 100~{cal_100}cal, 200~{cal_200}cal")
        print("=" * 160)
        print(
            f"{'Metric':<12} {'50TMA':<18} {'100TMA':<18} {'100EMA':<18} {'200TMA':<18} {'6M Ret':<18} {'12M Ret':<18} {'GLD Bench':<18}"
        )
        print("-" * 160)

        print(
            f"{'CAGR':<12} {ma50_metrics['cagr']:>16.2%} {ma100_metrics['cagr']:>16.2%} {ema100_metrics['cagr']:>16.2%} {ma200_metrics['cagr']:>16.2%} {ret6m_metrics['cagr']:>16.2%} {ret12m_metrics['cagr']:>16.2%} {gld_metrics['cagr']:>16.2%}"
        )
        print(
            f"{'Sharpe':<12} {ma50_metrics['sharpe']:>16.3f} {ma100_metrics['sharpe']:>16.3f} {ema100_metrics['sharpe']:>16.3f} {ma200_metrics['sharpe']:>16.3f} {ret6m_metrics['sharpe']:>16.3f} {ret12m_metrics['sharpe']:>16.3f} {gld_metrics['sharpe']:>16.3f}"
        )
        print(
            f"{'Max DD':<12} {ma50_metrics['max_drawdown']:>16.2%} {ma100_metrics['max_drawdown']:>16.2%} {ema100_metrics['max_drawdown']:>16.2%} {ma200_metrics['max_drawdown']:>16.2%} {ret6m_metrics['max_drawdown']:>16.2%} {ret12m_metrics['max_drawdown']:>16.2%} {gld_metrics['max_drawdown']:>16.2%}"
        )
        print("=" * 160)

        print("\nVs GLD Benchmark:")
        print(
            f"  50TMA:    {ma50_metrics['cagr'] - gld_metrics['cagr']:>+.2%} CAGR, Sharpe {ma50_metrics['sharpe'] - gld_metrics['sharpe']:>+.3f}"
        )
        print(
            f"  100TMA:   {ma100_metrics['cagr'] - gld_metrics['cagr']:>+.2%} CAGR, Sharpe {ma100_metrics['sharpe'] - gld_metrics['sharpe']:>+.3f}"
        )
        print(
            f"  100EMA:   {ema100_metrics['cagr'] - gld_metrics['cagr']:>+.2%} CAGR, Sharpe {ema100_metrics['sharpe'] - gld_metrics['sharpe']:>+.3f}"
        )
        print(
            f"  200TMA:   {ma200_metrics['cagr'] - gld_metrics['cagr']:>+.2%} CAGR, Sharpe {ma200_metrics['sharpe'] - gld_metrics['sharpe']:>+.3f}"
        )
        print(
            f"  6M Ret:   {ret6m_metrics['cagr'] - gld_metrics['cagr']:>+.2%} CAGR, Sharpe {ret6m_metrics['sharpe'] - gld_metrics['sharpe']:>+.3f}"
        )
        print(
            f"  12M Ret:  {ret12m_metrics['cagr'] - gld_metrics['cagr']:>+.2%} CAGR, Sharpe {ret12m_metrics['sharpe'] - gld_metrics['sharpe']:>+.3f}"
        )
