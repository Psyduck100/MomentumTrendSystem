...
def main() -> None:
    ...
    if include_set:
        bucket_map = {sym: bucket for sym, bucket in bucket_map.items() if bucket in include_set}
        tickers = [sym for sym in tickers if sym in bucket_map]
        if not tickers:
    ...
    if args.restrict_buckets:
        ...
        filtered_bucket_map = {sym: bucket for sym, bucket in filtered_bucket_map.items() if bucket in allowed}
        monthly = monthly[[sym for sym in monthly.columns if sym in filtered_bucket_map]]
    ...
    if args.holdings_output_dir:
        ...
        selected_keys = set(args.log_holdings) if args.log_holdings else {
            f"{results[0].rank_gap}:{results[0].filter_mode}"
        }
        for res in results:
            ...
            out_path = args.holdings_output_dir / f"holdings_rankgap{res.rank_gap}_{res.filter_mode}.csv"
