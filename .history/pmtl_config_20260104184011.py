...
class FallbackType(Enum):
    ...
    SPY_THEN_IEF = "spy_then_ief"  # New: fallback to SPY if 12m > 0, else IEF
...
class WindowConfig:
    ...
    start_window: int = 10  # Starting trading days
    end_window: int = 100  # Ending trading days
...
class DateConfig:
    ...
    end_date: str = "2023-12-31"  # Exclude 2024-2025 per request
...
class BacktestConfig:
    ...
    trailing_return_periods: Optional[list[int]] = None  # e.g., [6, 12]
    ...
    def __post_init__(self):
        ...
        if self.trailing_return_periods is None:
            self.trailing_return_periods = []
...
CASH_ONLY_CONFIG = BacktestConfig(
    fallback_type=FallbackType.CASH, output_prefix="pmtl_cash"
)
...
IEF_CONFIG = BacktestConfig(
    fallback_type=FallbackType.IEF, fallback_ticker="IEF", output_prefix="pmtl_ief"
)
...
# Test 9: Monthly, long-window sweep (100–200 by 10) with ret6/ret12
LONG_WINDOW_MONTHLY_CONFIG = BacktestConfig(
    fallback_type=FallbackType.CASH,
    dates=DateConfig(frequency="ME"),
    windows=WindowConfig(start_window=100, end_window=200, step=10),
    trailing_return_periods=[6, 12],
    output_prefix="pmtl_monthly_long",
)


# SPY-then-IEF fallback config (must be after class definitions)
SPY_THEN_IEF_CONFIG = BacktestConfig(
    fallback_type=FallbackType("spy_then_ief"),
    dates=DateConfig(frequency="ME"),
    windows=WindowConfig(start_window=100, end_window=200, step=10),
    output_prefix="pmtl_spy_then_ief",
)

# Active configuration (change this to switch tests)
ACTIVE_CONFIG = SPY_THEN_IEF_CONFIG
...
if __name__ == "__main__":
    ...
    print(
        f"  Windows: {ACTIVE_CONFIG.windows.start_window}-{ACTIVE_CONFIG.windows.end_window} (step {ACTIVE_CONFIG.windows.step})"
    )
    print(
        f"  Dates: {ACTIVE_CONFIG.dates.start_date} to {ACTIVE_CONFIG.dates.end_date}"
    )
