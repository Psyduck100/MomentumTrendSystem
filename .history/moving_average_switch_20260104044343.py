...
def run_strategy(
    ...
    if defensive_ticker:
        ...
        monthly_defensive = defensive.resample("ME").last().pct_change()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Moving-average stock/cash/bond switch"
    )
    parser.add_argument(
        "--stock", default="SPY", help="Broad stock ETF (e.g., SPY or VT)"
    )
    parser.add_argument(
        "--defensive",
        help="Optional defensive ETF (e.g., IEF); fallback to cash if omitted",
    )
    parser.add_argument("--start", default="2005-01-01")
    parser.add_argument("--end", default="2025-12-31")
    parser.add_argument(
        "--ma-length", type=int, default=200, help="Moving average length in days"
    )
    parser.add_argument(
        "--cash-rate",
        type=float,
        default=0.03,
        help="Annualized cash yield when defensive",
    )
...
def main() -> None:
    ...
    print("------------------------------")
    print(
        f"Period: {monthly_returns.index[0].date()} – {monthly_returns.index[-1].date()}"
    )
    print(
