...
def generate_configs() -> List[Dict[str, Any]]:
    ...
    gap_configs = [
        ...
        {"Bonds": 2, "Commodities": 1, "Emerging_Markets": 1, "International": 1, "US_equities": 1, "US_small_mid_cap": 1},
        {"Bonds": 1, "Commodities": 1, "Emerging_Markets": 1, "International": 1, "US_equities": 1, "US_small_mid_cap": 1},  # All gap=1
        {"Bonds": 2, "Commodities": 2, "Emerging_Markets": 2, "International": 2, "US_equities": 1, "US_small_mid_cap": 1},  # Bonds sticky, equities agile
        {"Bonds": 3, "Commodities": 2, "Emerging_Markets": 1, "International": 1, "US_equities": 0, "US_small_mid_cap": 0},  # Bonds very sticky, equities aggressive
        {"Bonds": 1, "Commodities": 2, "Emerging_Markets": 1, "International": 1, "US_equities": 2, "US_small_mid_cap": 2},  # Equities sticky, others agile
    ]
...
def walk_forward_test(
    ...
    test_years: int = 2,
    start_year: int = 2015,
    end_year: int = 2024,  # Only use complete years
) -> pd.DataFrame:
    """
    ...
    for window_idx, window in enumerate(windows):
        ...
        try:
            ...
            if test_metrics and test_metrics["n_months"] > 6:  # Need at least 6 months
                ...
                print(f"  Insufficient test data (only {test_metrics['n_months'] if test_metrics else 0} months)")
        except Exception as e:
...
if __name__ == "__main__":
    # Run walk-forward test with 4-year train, 2-year test windows
    results_df = walk_forward_test(
        train_years=4,
        test_years=2,
        start_year=2015,
        end_year=2024,  # Only use complete years (2025 is partial)
    )
