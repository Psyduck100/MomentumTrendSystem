...
def run_strategy(sma_window: int = SMA_WINDOW):
    ...
    for dt in month_starts:
        ...
        # Ensure scalar for price and SMA value
        price_val = prices.loc[dt]
        ...
        if pd.isna(sma_val):
            signals.append((dt, CASH_SYMBOL, price_val, sma_val, "No SMA yet"))
            continue
        if price_val > sma_val:
            signals.append((dt, TICKER, price_val, sma_val, "GLD above SMA: HOLD GOLD"))
        else:
            signals.append((dt, CASH_SYMBOL, price_val, sma_val, "GLD below SMA: HOLD CASH"))
    # Print summary
