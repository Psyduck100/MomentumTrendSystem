...
def backtest_vs_gld(sma_window: int = SMA_WINDOW):
    ...
    # Cumulative returns (start at 1, avoid NaN propagation)
    df_full["strat_cum"] = (1 + df_full["ret"]).cumprod()
    df_full["gld_cum"] = (1 + df_full["gld_ret"]).cumprod()
    # Remove any leading NaN from strat_cum (e.g., if strategy is in cash at start)
    first_valid = df_full["strat_cum"].first_valid_index()
    if first_valid is not None and first_valid != df_full.index[0]:
        df_full["strat_cum"] = df_full["strat_cum"].loc[first_valid:] / df_full["strat_cum"].loc[first_valid]
    # Plot
