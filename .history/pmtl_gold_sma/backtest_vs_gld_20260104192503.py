...
def backtest_vs_gld(sma_window: int = SMA_WINDOW):
    ...
    # If in cash, return is 0; if in GLD, return is GLD's daily return
    gld_rets = prices.pct_change().fillna(0)
    df_full["ret"] = 0.0
    df_full.loc[df_full["hold_gld"], "ret"] = gld_rets.loc[df_full["hold_gld"]]
    # Buy & hold GLD returns
    df_full["gld_ret"] = gld_rets
    # Cumulative returns (start at 1.0 from first valid period)
    strat_cum = (1 + df_full["ret"]).cumprod()
    # Find first period where strat_cum is not NaN or zero (should be first day, but robust)
    first_valid = strat_cum.first_valid_index()
    if first_valid is not None:
        strat_cum = strat_cum.loc[first_valid:]
        strat_cum = strat_cum / strat_cum.iloc[0]
        strat_cum.iloc[0] = 1.0
    df_full["strat_cum"] = strat_cum.reindex(df_full.index).fillna(method="ffill").fillna(1.0)
    df_full["gld_cum"] = (1 + df_full["gld_ret"]).cumprod().fillna(1.0)
    # Plot
