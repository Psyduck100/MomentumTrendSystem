...
ief_cagr = (
    sma_best["sma_cagr"]
    if sma_best["sma_cagr"] > ema_best["ema_cagr"]
    else ema_best["ema_cagr"]
)
ief_sharpe = (
    sma_best["sma_sharpe"]
    if sma_best["sma_cagr"] > ema_best["ema_cagr"]
    else ema_best["ema_sharpe"]
)
ief_maxdd = (
    sma_best["sma_maxdd"]
    if sma_best["sma_cagr"] > ema_best["ema_cagr"]
    else ema_best["ema_maxdd"]
)

print("\n📊 BEST STRATEGY BY DEFENSIVE ASSET")
print("-" * 110)
print(
    f"{'Defensive Asset':<20} {'Type':<6} {'Window':<8} {'CAGR':<12} {'Sharpe':<12} {'MaxDD':<12}"
)
print("-" * 110)
print(
    f"{'IEF (bonds)':<20} {ief_type:<6} {int(ief_best['window']):<8} {ief_cagr:>10.2%}  {ief_sharpe:>10.3f}  {ief_maxdd:>10.2%}"
)
print(
    f"{'CASH (0%)':<20} {cash.iloc[0]['type']:<6} {int(cash.iloc[0]['window']):<8} {cash.iloc[0]['cagr']:>10.2%}  {cash.iloc[0]['sharpe']:>10.3f}  {cash.iloc[0]['max_drawdown']:>10.2%}"
)
print(
    f"{'TB3MS (1.73%)':<20} {tb3ms.iloc[0]['type']:<6} {int(tb3ms.iloc[0]['window']):<8} {tb3ms.iloc[0]['cagr']:>10.2%}  {tb3ms.iloc[0]['sharpe']:>10.3f}  {tb3ms.iloc[0]['max_drawdown']:>10.2%}"
)
...
print(
    f"Sharpe Ratio Difference: {sharpe_diff:+.3f}  ({'CASH wins' if sharpe_diff > 0 else 'IEF wins'})"
)
print(
    f"Max Drawdown Difference: {dd_diff:+.2%}  ({'CASH better' if dd_diff < 0 else 'IEF better'} by {abs(dd_diff):.2%})"
)
...
print(
    f"CASH Performance: {cash_best['cagr']:.2%} CAGR with {cash_best['type']} {int(cash_best['window'])}"
)
