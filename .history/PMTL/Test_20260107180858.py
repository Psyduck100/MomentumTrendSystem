...
def monthly_eval_is_up(
    ...
    *,
    ...
    """
    Evaluate the state machine only on month-end closes, then forward-fill daily.
    Returns a DAILY is_up series suitable for backtesting with your existing code.
    """
    close = close.sort_index()

    # Month-end close series (last business day in month)
    close_m = close.resample("M").last().dropna()

    # Run your existing state machine on monthly data
    ...
    # Align back to daily index: carry the last monthly state forward until next month-end
    is_up_daily = is_up_m.reindex(close.index).ffill().fillna(False).astype(bool)
    return is_up_daily
...
def decision_gated_is_up_anysignal(
    ...
    *,
    ...
    feature_params: Dict[str, Any],
    rule_params: Dict[str, Any] | None = None,
    gate: str = "BM",  # business month-end
) -> pd.Series:
    """
    Daily features (so 260/90 remain DAYS), but allow state transitions only on gate days.
    Entry/exit are triggered if the signal fired at ANY time since the previous gate.
    """
    close_s = as_series(close, "Close").sort_index()
    feat = compute_features(close_s, **feature_params)
    ...
    # Gate days = last available close each period
    ...
    # We track whether we saw any entry/exit signal since the last gate
    pending_enter = False
    pending_exit = False
    ...
    for i in range(len(close_s)):
        ...
        # Accumulate signals during the interval
        if not in_up:
            if bool(enter.iat[i]):
                pending_enter = True
        else:
        ...
        # Apply transition only on gate day
        if is_gate_day[i]:
            ...
            # Reset pending flags after decision
            pending_enter = False
            pending_exit = False
...
def main():
    ...
    is_up = decision_gated_is_up(
        close,
        enter_rule=enter_breakout_ma_slope,
        exit_rule=exit_donchian_or_ma_buffer,
        feature_params=feature_params,  # keeps entry_len=260, exit_len=90 in DAYS
        rule_params=rule_params,
        gate="BM",  # business month-end is usually nicer than "M"
    )

    equity = backtest_long_only(close, is_up, fee_bps=2.0)

    print("Trades:", int((is_up.astype(int).diff().fillna(0) == 1).sum()))
    print("UP days:", int(is_up.sum()), "/", len(is_up))
    print("Equity final:", float(equity.iloc[-1]))
    print("CAGR:", cagr_from_equity(equity))
    print("MaxDD:", max_drawdown(equity))
    print("CAGR_in_pos:", in_position_cagr(close, is_up))
