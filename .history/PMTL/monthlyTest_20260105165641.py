...
def run_long_only_engine(
    ...
    *,
    name: str = "STRAT",
    flat_ret: Optional[pd.Series] = None,  # None => cash 0% when flat
):
    close = close.dropna()
    ...
    entry = entry_fn(close, params).reindex(close.index).fillna(False).astype(bool)
    exit_ = exit_fn(close, params).reindex(close.index).fillna(False).astype(bool)

    # position decided at bar t close
    ...
    # apply on next bar (no lookahead)
