...
def main():
    ...
    feature_params = dict(
        ...
        slope_lookback=20,
        entry_len=260,
        exit_len=90,  # exit channel length
    )
    ...
    monthly_eval_is_up = uptrend_state_machine(
        close,
        enter_rule=enter_breakout_ma_slope,
        exit_rule=exit_donchian_or_ma_buffer,
        feature_params=feature_params,
    ...
    periods = uptrend_periods(monthly_eval_is_up)
    ...
    print(f"UP days: {int(monthly_eval_is_up.sum())} / {len(monthly_eval_is_up)}")

    plot_uptrend(close, monthly_eval_is_up, ma_len=feature_params["ma_len"])
