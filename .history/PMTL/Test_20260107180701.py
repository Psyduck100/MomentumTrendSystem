...
def decision_gated_is_up(
    ...
    *,
    ...
    feature_params: Dict[str, Any],
    rule_params: Dict[str, Any] | None = None,
    gate: str = "M",  # "M" = month-end, "W-FRI" = weekly Friday, etc.
) -> pd.Series:
    """
    Compute signals on DAILY data (so entry_len/exit_len remain in DAYS),
    ...
    # readiness (same as your default rules need)
    ready = (
        (~feat["ma"].isna())
        & (~feat["ma_slope"].isna())
        & (~feat["ch_high_entry"].isna())
    ...
    # Gate dates: last available close in each gate period
    gate_index = close_s.resample(gate).last().index
    is_gate_day = close_s.index.isin(gate_index)
    ...
    for i in range(len(close_s)):
        ...
        # Only allow transitions on gate days
        if is_gate_day[i]:
            if (not in_up) and bool(enter.iat[i]):
                in_up = True
            elif in_up and bool(exit_.iat[i]):
