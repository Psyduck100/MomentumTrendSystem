...
def main():
    ...
    periods = {
        "2004_2012": ("2004-01-01", "2012-12-31"),
    ...
    for label, (start, end) in periods.items():
        if end is None:
            close_slice = close.loc[start:]
        else:
            close_slice = close.loc[start:end]
        if close_slice.empty:
            print(f"\nNo data for {label}; skipping.")
            continue

        res = sweep_down_regime(
            close_slice,
            gate="BME",
            ma_lens=(200, 210, 220, 230, 240, 250, 260, 270, 280, 290, 300, 310, 320, 330, 340, 350, 360, 270, 280, 390, 400),
            slope_lookback=20,
            exit_len=90,
        ...
        out_csv = f"gld_down_regime_optimize_{label}.csv"
