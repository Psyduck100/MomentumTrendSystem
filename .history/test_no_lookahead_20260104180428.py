...
def test_sma_signal_is_lagged_one_period():
    ...
    expected_index = pd.date_range("2020-01-05", periods=4, freq="W-SUN")
    expected = pd.Series(
        [np.nan, 0.0, -0.1818181818, 0.0], index=expected_index, name="SMA_2"
    )
...
def test_ema_signal_is_lagged_one_period():
    ...
    expected_index = pd.date_range("2020-01-05", periods=4, freq="W-SUN")
    expected = pd.Series(
        [np.nan, 0.0, -0.1818181818, 0.0], index=expected_index, name="EMA_2"
    )
