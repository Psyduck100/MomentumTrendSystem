...
def _write_excel_by_logic(df: pd.DataFrame, name: str) -> None:
    ...
    def _drop_redundant_columns(
        ...
        if exit_logic == "DONCHIAN":
            drop_cols += ["exit_len"]
        if entry_logic == "DONCHIAN" and exit_logic == "DONCHIAN":
            drop_cols += ["ma_buffer"]
...
def main() -> None:
    ...
    ma_buffers = (0.0, 0.0025, 0.005, 0.0075, 0.01)
    donchian_entry_buffers = (0.0,)
