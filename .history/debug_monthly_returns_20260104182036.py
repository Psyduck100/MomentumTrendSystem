...
if isinstance(data.columns, pd.MultiIndex):
    prices = (
        data[("Adj Close", main_ticker)]
        if ("Adj Close", main_ticker) in data.columns
        else data.iloc[:, 0]
    )
else:
...
print(
    f"   Mean: {main_monthly_ret.mean():.4f} ({main_monthly_ret.mean()*100:.2%} per month)"
)
...
print(
    f"   Mean: {blended_ret_series.mean():.4f} ({blended_ret_series.mean()*100:.2%} per month)"
)
