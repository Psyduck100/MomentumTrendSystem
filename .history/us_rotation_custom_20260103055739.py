...
Evaluation windows: Sequential 6-year spans, sequential 3-year spans, and rolling
5/10/15-year windows spanning 2001→2025.

For each window sweep momentum score modes, rank-gap thresholds, and absolute filters,
compare the strategy versus SPY/QQQ buy-and-hold, and export per-config monthly and
annual return series for downstream significance testing.
"""
...
def build_sequential_windows(ranges: list[tuple[str, int, int]]) -> list[tuple[str, str, str]]:
    windows: list[tuple[str, str, str]] = []
    for prefix, start_year, end_year in ranges:
        windows.append(
            (
                f"{prefix}_{start_year}_{end_year}",
                f"{start_year}-01-01",
                f"{end_year}-12-31",
            )
        )
...
WINDOWS = [
    ...
    *build_rolling_windows("5y", 5, 2001, 2025),
    *build_rolling_windows("10y", 10, 2001, 2025),
    *build_rolling_windows("15y", 15, 2001, 2025),
]
