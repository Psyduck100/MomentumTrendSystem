...
def compare_fallbacks():
    ...
    try:
        tbills = pd.read_csv("pmtl_tbills_results.csv").sort_values(
    ...
    print("-" * 80)
    print(
        f"{'Fallback':<15} {'Type':<6} {'Window':<8} {'CAGR':<12} {'Sharpe':<12} {'MaxDD':<12}"
    )
    print("-" * 80)

    c = cash.iloc[0]
    print(
        f"{'CASH':<15} {c['type']:<6} {int(c['window']):<8} {c['cagr']:>10.2%}  {c['sharpe']:>10.3f}  {c['max_drawdown']:>10.2%}"
    )

    if has_tbills:
        t = tbills.iloc[0]
        print(
            f"{'TB3MS':<15} {t['type']:<6} {int(t['window']):<8} {t['cagr']:>10.2%}  {t['sharpe']:>10.3f}  {t['max_drawdown']:>10.2%}"
        )
    ...
    if has_ief:
        i = ief.iloc[0]
        print(
            f"{'IEF':<15} {i['type']:<6} {int(i['window']):<8} {i['cagr']:>10.2%}  {i['sharpe']:>10.3f}  {i['max_drawdown']:>10.2%}"
        )
...
def summary_table():
    ...
    for results_file in sorted(results_files):
        ...
        print("-" * 80)
        print(
            f"{'Rank':<5} {'Type':<6} {'Window':<8} {'CAGR':<12} {'Sharpe':<12} {'MaxDD':<12}"
        )
        print("-" * 80)

        for rank, (idx, row) in enumerate(df_sorted.head(3).iterrows(), 1):
            print(
                f"{rank:<5} {row['type']:<6} {int(row['window']):<8} "
