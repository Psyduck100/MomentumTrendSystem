...
def backtest_exponential_moving_average(prices: pd.DataFrame) -> dict:
    ...
    # 100 trading days ≈ 144 calendar days for EMA span
    calendar_window = int(100 * TRADING_TO_CALENDAR)
    ema_column = "GLD_100EMA"
    prices[ema_column] = prices["GLD"].ewm(span=calendar_window, adjust=False).mean()
    monthly = prices.resample("ME").last()
...
def run_four_year_chunks() -> None:
    ...
    for start_year in start_years:
        ...
        print("="*140)
        header = f"{'Config':<10} {'CAGR':>10} {'Sharpe':>10} {'MaxDD':>10} {'ΔCAGR vs GLD':>15} {'ΔSharpe vs GLD':>15}"
        print(header)
        print("-"*140)
        for row in rows:
            print(
