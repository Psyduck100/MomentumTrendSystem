...
def download_prices(tickers: Sequence[str], start: str, end: str) -> pd.DataFrame:
    ...
    for i in range(0, len(tickers), chunk_size):
        ...
        print(
            f"Downloading {len(chunk)} tickers ({chunk[0]}..{chunk[-1]}) from {start} to {end}..."
        )
...
def _window_return(
    monthly: pd.DataFrame, idx: int, symbol: str, months: int
) -> float | None:
    if idx < months:
        return None
...
def run_two_stage_strategy(
    ...
    for idx in range(min_lookback, len(momentum) - 1):
        ...
        for symbol, bucket, _score in picks:
            filter_pass = passes_abs_filter(
                monthly, idx, symbol, filter_mode, filter_band
            )
            if filter_pass:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=[],
        help="If provided, only these buckets produce leaders (e.g., US_equities)",
    )
    parser.add_argument(
        "--rank-gaps",
        nargs="+",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Filter hurdle (e.g., 0.01 for +1%)",
    )
    parser.add_argument(
        "--top-k", type=int, default=2, help="Number of bucket winners to hold"
    )
    parser.add_argument(
        "--cash-rate", type=float, default=0.04, help="Annual cash rate when sidelined"
    )
    parser.add_argument(
        "--slippage-bps", type=float, default=3.0, help="Per-trade slippage in bps"
    )
    parser.add_argument(
        "--expense-ratio",
        type=float,
        default=0.001,
        help="Annual expense drag applied when invested",
    )
    parser.add_argument("--cache-dir", type=Path, default=Path("backtest_cache"))
    parser.add_argument(
        "--output", type=Path, help="Optional CSV to store summary metrics"
    )
    parser.add_argument(
        "--holdings-output-dir",
        type=Path,
        help="If set, dump holdings timeline per config into this directory",
    )
    parser.add_argument(
        "--log-holdings",
        nargs="*",
        default=[],
        help="Specific configs to log as rank_gap:filter (e.g., 0:none)",
    )
...
def main() -> None:
    ...
    if invalid_filters:
        raise ValueError(
            f"Unknown filter mode(s): {', '.join(sorted(invalid_filters))}"
        )
    ...
    if args.exclude_buckets:
        ...
        bucket_map = {
            sym: bucket for sym, bucket in bucket_map.items() if bucket not in excluded
        }
    ...
    include_set = set(args.restrict_buckets)
    if include_set:
        bucket_map = {
            sym: bucket for sym, bucket in bucket_map.items() if bucket in include_set
        }
        tickers = [sym for sym in tickers if sym in bucket_map]
        if not tickers:
            raise RuntimeError("No tickers left after applying --restrict-buckets")
        print(f"Restricting to buckets: {', '.join(sorted(include_set))}")
    ...
    if args.restrict_buckets:
        allowed = set(args.restrict_buckets)
        filtered_bucket_map = {
            sym: bucket
            for sym, bucket in filtered_bucket_map.items()
            if bucket in allowed
        }
        monthly = monthly[
            [sym for sym in monthly.columns if sym in filtered_bucket_map]
        ]
    ...
    if args.holdings_output_dir:
        args.holdings_output_dir.mkdir(parents=True, exist_ok=True)
        selected_keys = (
            set(args.log_holdings)
            if args.log_holdings
            else {f"{results[0].rank_gap}:{results[0].filter_mode}"}
        )
        for res in results:
            key = f"{res.rank_gap}:{res.filter_mode}"
            if key not in selected_keys:
                continue
            df_holdings = pd.DataFrame(
                {
                    "date": res.returns.index[: len(res.holdings)],
                    "symbols": ["|".join(pos) if pos else "" for pos in res.holdings],
                }
            )
            out_path = (
                args.holdings_output_dir
                / f"holdings_rankgap{res.rank_gap}_{res.filter_mode}.csv"
            )
            df_holdings.to_csv(out_path, index=False)
            print(f"Saved holdings timeline to {out_path}")
