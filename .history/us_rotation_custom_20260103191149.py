...
REFINED_ALPHA_GRID = [round(1.0 + 0.05 * i, 2) for i in range(int((2.0 - 1.0) / 0.05) + 1)]
...
def run_walk_forward_splits(
    ...
    combo_defs = build_combo_defs(score_configs, rank_gaps=rank_gaps, filter_modes=filter_modes)
    print(
    ...
    for idx, block in enumerate(splits, start=1):
        ...
        for score_config, gap, filter_mode in combo_defs:
            ...
            if test_snapshot is not None:
                ...
                if bench and "max_drawdown" in bench:
                    gap_value = test_snapshot["metrics"]["max_drawdown"] - bench["max_drawdown"]
        ...
        if test_candidates:
            test_candidates.sort(
                key=lambda c: c["test"].get("excess_cagr")
    ...
    aggregate_rows.sort(
        key=lambda row: row["avg_test_excess"] if row["avg_test_excess"] is not None else float("-inf"),
        reverse=True,
    )
...
def compute_plateau_selection(
    ...
    def complexity_key(row: dict[str, Any]) -> tuple[float, float, float]:
        ...
        param_abs = abs(float(param_value)) if isinstance(param_value, (int, float)) else float("inf")
...
def run_walk_forward_analysis() -> dict[str, Any]:
    ...
    if REFINED_ALPHA_CONFIGS:
        ...
        refined_plateau = compute_plateau_selection(refined_output["aggregate"]["per_config"])
