...
ALL_TICKERS = US_TICKERS + PROXY_TICKERS + INTL_TICKERS + BONDS
...
def backtest_layered(prices: pd.DataFrame) -> dict:
    """
    Layer 1: US bucket rotation (blend_6_12 + ret_12m filter)
    Layer 2: INTL fallback (EFA/ACWX composite)
    Layer 3: Bonds (IEF)
    """
    ...
    for i in range(1, len(returns["blend_6_12"])):
        # Layer 1: Find US winner by blend_6_12 score
        blend_scores = returns["blend_6_12"].iloc[i][us_tickers]
        us_winner = blend_scores.idxmax()
        us_winner_12m = returns["ret_12m"].iloc[i][us_winner]

        # Layer 1 filter: US winner's 12M return > 0?
        if us_winner_12m > 0:
            position = us_winner
        else:
            # Layer 2: Check INTL
            intl_12m = returns["ret_12m"].iloc[i]["INTL"]
            if intl_12m > 0:
                position = "INTL"
            else:
                # Layer 3: Bonds
    ...
    return_series = pd.Series(
        monthly_rets, index=monthly.index[2:len(monthly)]
    )
    return {
        "label": "layered_us_intl_bonds",
        "positions": positions,
