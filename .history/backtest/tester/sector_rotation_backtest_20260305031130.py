...
def _pick_with_rank_gap(
    scores: pd.Series, current_asset: str | None, rank_gap: int
) -> str:
...
def run_rotation(
    ...
    # Use all provided columns except the risk-off asset as the live risk-on universe
    risk_on_universe = [t for t in prices.columns if t != RISK_OFF]
    if RISK_OFF not in prices.columns:
        raise ValueError(
            f"{RISK_OFF} must be included in prices for risk-off behavior."
        )
    ...
    for i, date in enumerate(dates):
        ...
        asset_ret = (
            0.0 if current_asset is None else float(returns.at[date, current_asset])
        )
        ...
        scores = _momentum_scores(
            prices[risk_on_universe], date, lookback_days=lookback_days
        )
        if scores.empty:
            continue
        ...
        decision_rows.append(
            {
                ...
                "leader_minus_chosen_mom": (
                    (leader_mom - chosen_mom) if pd.notna(chosen_mom) else np.nan
                ),
    ...
    return RotationResult(
        daily=daily, decisions=decisions, trades=trades, metrics=metrics
    )
...
def _hold_stats(daily: pd.DataFrame, prices: pd.DataFrame) -> pd.DataFrame:
        ...
        rows.append(
            {
                ...
                "hold_days": hold_days,
                "hold_%_all_days": (
                    (hold_days / total_days) * 100.0 if total_days else np.nan
                ),
                "hold_%_invested_days": (
                    (hold_days / invested_days) * 100.0 if invested_days else np.nan
                ),
                "avg_daily_asset_ret_%": (
                    avg_asset_ret * 100.0 if pd.notna(avg_asset_ret) else np.nan
                ),
                "avg_daily_strat_ret_%": (
                    avg_strat_ret * 100.0 if pd.notna(avg_strat_ret) else np.nan
                ),
                "ann_asset_ret_when_held_%": (
                    ann_asset_ret * 100.0 if pd.notna(ann_asset_ret) else np.nan
                ),
            }
        )
    ...
    return (
        pd.DataFrame(rows)
        .sort_values("hold_%_all_days", ascending=False)
...
def _leave_one_out(
        ...
        rows.append(
            {
                ...
                "delta_maxdd_pp": (test.metrics["maxdd"] - base.metrics["maxdd"])
                * 100.0,
                "delta_risk_on_pp": test.metrics["risk_on_%"]
                - base.metrics["risk_on_%"],
                "delta_bonds_pp": test.metrics["bonds_%"] - base.metrics["bonds_%"],
                "hurt_if_removed_positive": (
                    test.metrics["cagr"] > base.metrics["cagr"]
                ),
            }
        )
    ...
    return (
        pd.DataFrame(rows)
        .sort_values("delta_cagr_pp", ascending=False)
        .reset_index(drop=True)
    )


def _benchmark_report(
    start: str, end: str | None, idx: pd.DatetimeIndex
) -> tuple[dict, dict]:
    bench_total = _download_prices([BENCHMARK], start=start, end=end, auto_adjust=True)[
        BENCHMARK
    ]
    bench_price = _download_prices(
        [BENCHMARK], start=start, end=end, auto_adjust=False
    )[BENCHMARK]
...
def _rank_gap_comparison(base: RotationResult, alt: RotationResult) -> dict[str, float]:
    ...
    rel_cagr = (
        float(rel.iloc[-1] ** (1.0 / years) - 1.0) if years and years > 0 else np.nan
    )
...
def _decision_regret(
    decisions: pd.DataFrame, prices: pd.DataFrame, trade_delay: int = 1
) -> pd.DataFrame:
    if decisions.empty:
        return pd.DataFrame(
            columns=[
    ...
    for i, row in decisions.reset_index(drop=True).iterrows():
        ...
        chosen_ret = float(
            prices.at[end_date, chosen] / prices.at[start_date, chosen] - 1.0
        )
        leader_ret = float(
            prices.at[end_date, leader] / prices.at[start_date, leader] - 1.0
        )
        rows.append(
            {
                "date": d,
...
def _information_ratio(
    strategy_ret: pd.Series, benchmark_ret: pd.Series, ann: int = 252
) -> float:
...
def _rolling_information_ratio(
    ...
    def _ir(x: pd.Series) -> float:
        std = float(x.std(ddof=1))
        if std <= 0.0:
            return np.nan
        return float(np.sqrt(ann) * x.mean() / std)

    return active.rolling(window=window, min_periods=window).apply(
        lambda v: _ir(pd.Series(v)),
        raw=False,
    )


def _rolling_max_drawdown(strategy_ret: pd.Series, window: int = 252 * 3) -> pd.Series:
    ret = strategy_ret.fillna(0.0)
    ...
    return ret.rolling(window=window, min_periods=window).apply(
        lambda v: _mdd(pd.Series(v)),
        raw=False,
    )
...
def main() -> None:
    ...
    spy_total = _download_prices(
        [BENCHMARK], start=start_date, end=end_date, auto_adjust=True
    )[BENCHMARK]
    ...
    loo_gap0 = _leave_one_out(
        prices, lookback_days, rank_gap=0, cost_bps=cost_bps, slippage_bps=slippage_bps
    )
    loo_gap1 = _leave_one_out(
        prices, lookback_days, rank_gap=1, cost_bps=cost_bps, slippage_bps=slippage_bps
    )

    comp = _rank_gap_comparison(run_gap0, run_gap1)

    bench_price, bench_total = _benchmark_report(
        start_date, end_date, run_gap0.daily.index
    )
    bench_total_ret = (
        _download_prices([BENCHMARK], start=start_date, end=end_date, auto_adjust=True)[
            BENCHMARK
        ]
        .pct_change()
        .fillna(0.0)
    ...
    kept_non_leader_days = (
        int(run_gap1.decisions["kept_non_leader"].sum())
        if not run_gap1.decisions.empty
        else 0
    )
    risk_off_switches0 = (
        int((run_gap0.decisions["risk_mode"] == "RISK_OFF").sum())
        if not run_gap0.decisions.empty
        else 0
    )
    risk_off_switches1 = (
        int((run_gap1.decisions["risk_mode"] == "RISK_OFF").sum())
        if not run_gap1.decisions.empty
        else 0
    )

    regime = (
        _regime_labels_from_spy(spy_total).reindex(run_gap0.daily.index).ffill().bfill()
    )
    ...
    avg_regret_all_gap0 = (
        float(regret_gap0["leader_minus_chosen_ret"].mean())
        if not regret_gap0.empty
        else np.nan
    )
    avg_regret_all_gap1 = (
        float(regret_gap1["leader_minus_chosen_ret"].mean())
        if not regret_gap1.empty
        else np.nan
    )
    avg_regret_non_leader_gap1 = (
        float(
            regret_gap1.loc[
                regret_gap1["kept_non_leader"], "leader_minus_chosen_ret"
            ].mean()
        )
    ...
    window_3y = 252 * 3
    roll_ir_on = _rolling_information_ratio(
        run_gap1_no_xly_def_on.daily["ret"],
        bench_total_ret,
        window=window_3y,
    )
    roll_ir_off = _rolling_information_ratio(
        run_gap1_no_xly_def_off.daily["ret"],
        bench_total_ret,
        window=window_3y,
    )
    roll_mdd_on = _rolling_max_drawdown(
        run_gap1_no_xly_def_on.daily["ret"], window=window_3y
    )
    roll_mdd_off = _rolling_max_drawdown(
        run_gap1_no_xly_def_off.daily["ret"], window=window_3y
    )

    sum_roll_ir_on = _series_summary(roll_ir_on)
    sum_roll_ir_off = _series_summary(roll_ir_off)
    sum_roll_mdd_on = _series_summary(roll_mdd_on)
    sum_roll_mdd_off = _series_summary(roll_mdd_off)
    ...
    print(
        f"Period: {run_gap0.daily.index.min().date()} -> {run_gap0.daily.index.max().date()}"
    )
    print(
        f"Lookback days: {lookback_days} | Costs: {cost_bps + slippage_bps:.1f} bps turnover"
    )
    ...
    print("\nSPY benchmark")
    print(
        f"  Price-only: CAGR {bench_price['cagr']:.2%} | Sharpe {bench_price['sharpe']:.2f} | MaxDD {bench_price['maxdd']:.2%} | Total {bench_price['total_return']:.2%}"
    )
    print(
        f"  Total-return: CAGR {bench_total['cagr']:.2%} | Sharpe {bench_total['sharpe']:.2f} | MaxDD {bench_total['maxdd']:.2%} | Total {bench_total['total_return']:.2%}"
    )
    ...
    cagr_delta = (
        run_gap1_no_xly_def_off.metrics["cagr"] - run_gap1_no_xly_def_on.metrics["cagr"]
    ) * 100.0
    maxdd_delta = (
        run_gap1_no_xly_def_off.metrics["maxdd"]
        - run_gap1_no_xly_def_on.metrics["maxdd"]
    ) * 100.0
    ...
    print("\nRolling 3Y stability (rank_gap=1, XLY removed; vs SPY total-return)")
    print("  Rolling IR - Defensive ON")
    print(
        f"    Latest: {sum_roll_ir_on['latest']:.3f} | Median: {sum_roll_ir_on['median']:.3f} | "
        f"P10/P90: {sum_roll_ir_on['p10']:.3f}/{sum_roll_ir_on['p90']:.3f} | Negative windows: {sum_roll_ir_on['neg_%']:.1f}%"
    )
    print("  Rolling IR - Defensive OFF")
    print(
        f"    Latest: {sum_roll_ir_off['latest']:.3f} | Median: {sum_roll_ir_off['median']:.3f} | "
        f"P10/P90: {sum_roll_ir_off['p10']:.3f}/{sum_roll_ir_off['p90']:.3f} | Negative windows: {sum_roll_ir_off['neg_%']:.1f}%"
    )

    print("  Rolling 3Y MaxDD - Defensive ON")
    print(
        f"    Latest: {sum_roll_mdd_on['latest']:.2%} | Median: {sum_roll_mdd_on['median']:.2%} | "
        f"P10/P90: {sum_roll_mdd_on['p10']:.2%}/{sum_roll_mdd_on['p90']:.2%}"
    )
    print("  Rolling 3Y MaxDD - Defensive OFF")
    print(
        f"    Latest: {sum_roll_mdd_off['latest']:.2%} | Median: {sum_roll_mdd_off['median']:.2%} | "
        f"P10/P90: {sum_roll_mdd_off['p10']:.2%}/{sum_roll_mdd_off['p90']:.2%}"
    )
