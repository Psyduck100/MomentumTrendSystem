...
def main() -> None:
    ...
    # Build weighted buy-and-hold baseline
    # For each asset, we calculate the return earned during the days it was held
    # Then weight by those days
    returns = prices_no_xly.pct_change().fillna(0.0)
    held_returns = []

    for asset in hold_stats.keys():
        hold_days = hold_stats[asset]
        if hold_days == 0:
            continue

        # Get indices where this asset was held
        held_mask = daily_df["held_asset"] == asset
        held_dates = daily_df.loc[held_mask].index

        if len(held_dates) == 0:
            continue

        # Sum returns during held period
        asset_returns_during_hold = returns.loc[held_dates, asset]
        cumulative_return = (1.0 + asset_returns_during_hold).prod() - 1.0

        held_returns.append({
            "asset": asset,
            "days_held": hold_days,
            "cumulative_return": cumulative_return,
            "weight": hold_days / len(daily_df),
        })

    bh_df = pd.DataFrame(held_returns)
    print("Buy-and-hold returns by asset (during same held periods):")
    print(
        bh_df[["asset", "days_held", "cumulative_return"]]
        .to_string(
            index=False,
    ...
    # Weighted buy-and-hold return
    bh_weighted_return = float((bh_df["cumulative_return"] * bh_df["weight"]).sum())

    # Rotation return from daily equity curve
    rotation_total_return = float(rotation.metrics["total_return"])
    ...
    for asset in hold_stats.keys():
        held_mask = daily_df["held_asset"] == asset
        bh_daily_ret.loc[held_mask] = returns.loc[held_mask, asset].values
    ...
    print(f"\nWeighted Buy-and-Hold (same hold days per asset):")
