...
def compute_relative_metrics(
    ...
    bench_var = (
        float(aligned["benchmark"].var(ddof=1)) if len(aligned) > 1 else float("nan")
    )
    cov = (
...
def format_metrics_block(
    metrics: dict[str, float], *, include_relative: bool = False
) -> str:
    lines = [
    ...
    if not pd.isna(metrics.get("time_in_position", float("nan"))):
        lines.append(
            f"Time in Position: {_fmt_pct(metrics.get('time_in_position', float('nan')))}"
        )
    if include_relative:
        lines.extend(
            [
