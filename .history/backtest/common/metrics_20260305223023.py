...
def format_metrics_block(metrics: dict[str, float], *, include_relative: bool = False) -> str:
    lines = [
        ...
        f"Calmar: {_fmt_num(metrics.get('calmar', float('nan')))}",
        f"Total Return: {_fmt_pct(metrics.get('total_return', float('nan')))}",
        f"Hit Rate: {_fmt_pct(metrics.get('hit_rate', float('nan')))}",
    ]
    ...
    if include_relative:
        lines.extend(
            [
                ...
                f"Tracking Error: {_fmt_pct(metrics.get('tracking_error', float('nan')))}",
                f"Active Return: {_fmt_pct(metrics.get('active_return', float('nan')))}",
                f"Beta vs Benchmark: {_fmt_num(metrics.get('beta_to_benchmark', float('nan')), 3)}",
                f"Correlation vs Benchmark: {_fmt_num(metrics.get('correlation', float('nan')), 3)}",
            ]
        )
    return "\n".join(lines)
