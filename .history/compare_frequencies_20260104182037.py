...
monthly_ema100 = monthly[(monthly["type"] == "EMA") & (monthly["window"] == 100)].iloc[
    0
]
weekly_ema100 = weekly[(weekly["type"] == "EMA") & (weekly["window"] == 100)].iloc[0]
biweekly_ema100 = biweekly[
    (biweekly["type"] == "EMA") & (biweekly["window"] == 100)
].iloc[0]
...
print(
    f"{'Monthly':<15} {monthly_ema100['cagr']:>10.2%}  {monthly_ema100['sharpe']:>10.3f}  "
    f"{monthly_ema100['max_drawdown']:>10.2%}  {'12':<12}"
)
print(
    f"{'Biweekly':<15} {biweekly_ema100['cagr']:>10.2%}  {biweekly_ema100['sharpe']:>10.3f}  "
    f"{biweekly_ema100['max_drawdown']:>10.2%}  {'26':<12}"
)
print(
    f"{'Weekly':<15} {weekly_ema100['cagr']:>10.2%}  {weekly_ema100['sharpe']:>10.3f}  "
...
print(
    f"   Biweekly:  {biweekly_ema100['cagr']:.2%} ({biweekly_vs_monthly_cagr:+.2%} vs monthly)"
)
print(
    f"   Weekly:    {weekly_ema100['cagr']:.2%} ({weekly_vs_monthly_cagr:+.2%} vs monthly)"
)
...
print(
    f"   But max drawdown is worse: {weekly_ema100['max_drawdown']:.2%} vs {monthly_ema100['max_drawdown']:.2%}"
)
...
for _, row in monthly_sorted.iterrows():
    print(
        f"  {row['type']} {int(row['window']):3d}: {row['cagr']:6.2%} CAGR, "
