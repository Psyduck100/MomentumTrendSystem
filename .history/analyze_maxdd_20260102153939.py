...
def analyze_us_equities_trades():
    ...
    for i in range(1, len(dd_gap0) - 1):
        if dd_gap0.iloc[i] < -0.10 and dd_gap0.iloc[i] < dd_gap0.iloc[i-1] and dd_gap0.iloc[i] <= dd_gap0.iloc[i+1]:
            significant_dds_gap0.append((i, dd_gap0.iloc[i]))
    
    for i in range(1, len(dd_gap3) - 1):
        if dd_gap3.iloc[i] < -0.10 and dd_gap3.iloc[i] < dd_gap3.iloc[i-1] and dd_gap3.iloc[i] <= dd_gap3.iloc[i+1]:
    ...
    for idx, dd_val in significant_dds_gap0[:3]:  # Analyze top 3 gap=0 drawdowns
        ...
        if recent_switches:
            ...
            print(f"  Gap=0: {pos0} (ret: {ret0:.2%}) <-- Switched from {prev} {lookback} month(s) ago")
        else:
            print(f"  Gap=0: {pos0} (ret: {ret0:.2%}) [no recent switch]")
        print(f"  Gap=3: {pos3} (ret: {ret3:.2%})")
        diff = ret3 - ret0
        if abs(diff) > 0.02:  # Material difference
            print(f"  Performance diff: {diff:.2%} {'✓ Gap=3 avoided loss' if diff > 0 else '✗ Gap=3 did worse'}")
        else:
