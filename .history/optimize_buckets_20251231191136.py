...
def test_bucket_combinations() -> None:
    ...
    print(
        f"Testing bucket combinations with params: lookback={best_params['lookback']}M, "
    ...
    baseline_result = run_backtest_with_buckets(
        tickers, full_bucket_map, all_buckets, test_start, test_end, best_params, cfg
    )
    results.append(
        {
            "config": "All buckets",
    ...
    print(
        f"   CAGR: {baseline_result['cagr']:.2%}, Sharpe: {baseline_result['sharpe']:.2f}, "
    ...
    for bucket_to_remove in all_buckets:
        ...
        print(
            f"      CAGR: {result['cagr']:.2%} ({cagr_diff:+.2%}), "
        ...
        core_combos = [
            {
                ...
                "buckets": [
                    ...
                    if any(
                        x in b for x in ["US", "International", "Developed", "Emerging"]
                    )
                ],
            },
            {
    ...
    improvements.sort(
        key=lambda x: x["sharpe"] - baseline_result["sharpe"], reverse=True
    )
