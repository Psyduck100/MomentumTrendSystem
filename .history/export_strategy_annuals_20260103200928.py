...
def run_strategy(spec: StrategySpec) -> tuple[pd.Series, pd.Series, pd.Series]:
    ...
    return compound_by_year(monthly), compound_by_year(spy_monthly), compound_by_year(qqq_monthly)
...
def main() -> None:
    ...
    all_years = sorted(
        set().union(*(series.index.tolist() for series in annual_data.values()), spy_annual.index.tolist())
    )
