...
def compute_benchmark_cagr(ticker: str, start: str, end: str) -> float:
    ...
    try:
        ...
        print(f"Downloaded {ticker}: {len(data)} rows, columns={list(data.columns)}")
        
        if data.empty:
            print(f"  {ticker}: Empty data")
            return float("nan")
        
        # For single ticker download, yfinance returns simple columns not MultiIndex
        ...
        print(f"  {ticker}: {len(prices)} prices after dropna, range {prices.index[0]} to {prices.index[-1]}")
        
        if len(prices) < 2:
            print(f"  {ticker}: Too few prices ({len(prices)})")
            return float("nan")
        ...
        print(f"  {ticker}: start=${start_price:.2f}, end=${end_price:.2f}, years={years:.2f}")
        
        if start_price <= 0 or years <= 0:
            print(f"  {ticker}: Invalid start price or years")
            return float("nan")
        
        total_return = (end_price / start_price) - 1
        cagr = (1 + total_return) ** (1 / years) - 1
        print(f"  {ticker}: CAGR = {cagr:.4f} ({cagr*100:.2f}%)")
        return cagr
