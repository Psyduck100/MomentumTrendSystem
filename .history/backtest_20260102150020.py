...
def run_backtest() -> None:
    ...
    # Down-sample per-bucket combos to keep runtime reasonable (reproducible sample)
    max_gap_combos = 40
    if len(gap_combinations) > max_gap_combos:
        rng = random.Random(42)
        gap_combinations = rng.sample(gap_combinations, k=max_gap_combos)
    ...
    with gap_stress_path.open("w") as f:
        ...
        for abs_mode, abs_band in abs_filters:
            ...
            for bucket in all_bucket_names:
                f.write(f"{bucket}:\n")
                f.write(f"  Gap | CAGR    | Sharpe | MaxDD   | (from uniform gap configs)\n")
                f.write(f"  ----|---------|--------|---------|---\n")
                ...
                for gap_val in [0, 1, 2, 3]:
                    if gap_metrics[gap_val]:
                        ...
                        f.write(f"   {gap_val}  | {m['cagr']:7.2%} | {m['sharpe']:6.2f} | {m['max_drawdown']:7.2%} |\n")
                    else:
