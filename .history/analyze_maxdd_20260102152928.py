...
def analyze_us_equities_trades():
    ...
    # Extract US_equities positions
    positions_gap0 = result_gap0['bucket_positions']['US_equities']
    positions_gap3 = result_gap3['bucket_positions']['US_equities']
    
    # Extract US_equities returns
    returns_gap0 = result_gap0['bucket_returns']['US_equities']
    returns_gap3 = result_gap3['bucket_returns']['US_equities']
    ...
    for i in range(min(36, len(positions_gap0), len(positions_gap3))):
        pos0 = positions_gap0[i][0] if positions_gap0[i] else None
        pos3 = positions_gap3[i][0] if positions_gap3[i] else None
        same = "Yes" if pos0 == pos3 else "No"
        
        ret0 = f"{returns_gap0[i]['return']:.2%}" if i < len(returns_gap0) else "N/A"
        ret3 = f"{returns_gap3[i]['return']:.2%}" if i < len(returns_gap3) else "N/A"
        
        print(f"{i:<6} {pos0 or 'None':<10} {pos3 or 'None':<10} {same:<8} {ret0:<15} {ret3:<15}")
    
    # Calculate cumulative returns and find worst drawdowns
    df_gap0 = pd.DataFrame(returns_gap0)
    df_gap3 = pd.DataFrame(returns_gap3)
    ...
    start_idx = max(0, worst_dd0_idx - 5)
    end_idx = min(len(positions_gap0), worst_dd0_idx + 6)
    for i in range(start_idx, end_idx):
        pos = positions_gap0[i][0] if positions_gap0[i] else None
        ret = returns_gap0[i]['return']
        dd = dd_gap0.iloc[i]
        marker = " <-- WORST DD" if i == worst_dd0_idx else ""
        print(f"  Month {i}: Holding {pos}, Return {ret:.2%}, DD {dd:.2%}{marker}")
    
    print(f"\nGap=3: MaxDD = {dd_gap3.min():.2%} at month {worst_dd3_idx}")
    print(f"  Period around worst DD:")
    start_idx = max(0, worst_dd3_idx - 5)
    end_idx = min(len(positions_gap3), worst_dd3_idx + 6)
    for i in range(start_idx, end_idx):
        pos = positions_gap3[i][0] if positions_gap3[i] else None
        ret = returns_gap3[i]['return']
        dd = dd_gap3.iloc[i]
        marker = " <-- WORST DD" if i == worst_dd3_idx else ""
        print(f"  Month {i}: Holding {pos}, Return {ret:.2%}, DD {dd:.2%}{marker}")
