...
def analyze_us_equities_trades():
    ...
    for i in range(min(36, len(positions_gap0), len(positions_gap3), len(returns_gap0), len(returns_gap3))):
        pos0 = positions_gap0[i][0] if len(positions_gap0[i]) > 0 else None
        pos3 = positions_gap3[i][0] if len(positions_gap3[i]) > 0 else None
        same = "Yes" if pos0 == pos3 else "No"
        
        ret0 = f"{returns_gap0[i]['return']:.2%}"
        ret3 = f"{returns_gap3[i]['return']:.2%}"
