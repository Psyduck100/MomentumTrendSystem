...
class TBillsFallback(FallbackStrategy):
    ...
    def get_monthly_returns(self, monthly_dates: pd.DatetimeIndex) -> pd.Series:
        ...
        return pd.Series(tbill_monthly.values, index=monthly_dates, name="tbills_returns")
...
class YFinanceFallback(FallbackStrategy):
    ...
    def _load_prices(self):
        ...
        if isinstance(data.columns, pd.MultiIndex):
            self._data = data[('Close', self.ticker)] if ('Close', self.ticker) in data.columns else data.iloc[:, 0]
        else:
    ...
    def get_monthly_returns(self, monthly_dates: pd.DatetimeIndex) -> pd.Series:
        ...
        return pd.Series(result.values, index=monthly_dates, name=f"{self.ticker.lower()}_returns")
