...
def load_bucket(bucket_dir: Path, bucket_name: str) -> tuple[list[str], dict[str, str]]:
    ...
    tickers = [t for t in provider.get_tickers() if provider.get_bucket_map().get(t) == bucket_name]
    if not tickers:
...
def softmax(scores: pd.Series, temperature: float, top_n: int | None = None) -> pd.Series:
...
def run_strategy(
    ...
    for idx in range(min_idx, len(monthly) - 1):
        ...
        if weights.empty:
            continue
        weighted_ret12 = float((ret12.iloc[idx].reindex(weights.index).fillna(0) * weights).sum())
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--temperature", type=float, default=0.5, help="Softmax temperature (lower=sharper)")
    parser.add_argument("--top-n", type=int, default=0, help="Limit softmax to top-N scores (0=all)")
    parser.add_argument("--abs-threshold", type=float, default=0.0, help="Weighted 12M hurdle to stay invested")
    parser.add_argument("--cash-rate", type=float, default=0.04, help="Annual cash return when defensive")
...
def main() -> None:
    ...
    returns, weights = run_strategy(
        ...
        top_n=top_n if top_n is not None else len(tickers),
