...
def run_walk_forward_bucket_comparison() -> None:
    ...
    for config_name, buckets_to_include in configs.items():
        ...
        for fold_idx, fold in enumerate(folds, 1):
            ...
            overall_positions = backtest_data["overall_positions"]
            ...
            if not overall_positions.empty:
                ...
                test_positions = overall_positions.loc[mask_pos]
            else:
                test_positions = overall_positions

            # If we have no usable data (e.g., early folds with few tickers), skip
            if test_returns.empty or test_positions.empty:
    ...
    if "WITH REITs" in results and "WITHOUT REITs" in results:
        ...
        # Align by year to avoid silent truncation when fold counts differ
        by_year_with = {row["year"]: row for row in with_reits_folds}
        by_year_without = {row["year"]: row for row in without_reits_folds}
        all_years = sorted(set(by_year_with) | set(by_year_without))
        ...
        for year in all_years:
            wr = by_year_with.get(year)
            wo = by_year_without.get(year)
            wr_sharpe = wr["sharpe"] if wr else float("nan")
            wo_sharpe = wo["sharpe"] if wo else float("nan")
            diff = wo_sharpe - wr_sharpe if not np.isnan(wr_sharpe) and not np.isnan(wo_sharpe) else float("nan")
            wr_txt = f"{wr_sharpe:.2f}" if wr else "-"
            wo_txt = f"{wo_sharpe:.2f}" if wo else "-"
            diff_txt = f"{diff:+.2f}" if not np.isnan(diff) else "-"
            print(f"{year:<20} {wr_txt:<20} {wo_txt:<20} {diff_txt:<15}")
