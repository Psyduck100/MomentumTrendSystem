...
def restrict_to_index(px_monthly: pd.DataFrame, idx: pd.DatetimeIndex) -> pd.DataFrame:
    px = px_monthly.reindex(idx)
...
# ----------------------------
# Compare universes on SAME window
# ----------------------------
...
def run_one_on_prices(cfg: StrategyConfig, px_monthly: pd.DataFrame) -> Dict[str, Any]:
...
def main():
    ...
    # Download ONCE: union of all tickers needed
    all_needed = set()
    for c in cfgs:
        all_needed |= set(c.universe)
        all_needed.add(c.defensive)
        all_needed.add("SPY")

    all_needed = sorted(all_needed)

    px_daily = download_prices(
        all_needed, start_date="2000-01-01", end_date=None, auto_adjust=True
    )
    px_monthly_all = to_monthly_prices(px_daily)

    # Find the TRUE intersection window across ALL tickers used in either run
    # (apples-to-apples window)
    px_monthly_common = px_monthly_all.dropna(how="any")
    if len(px_monthly_common) == 0:
        raise ValueError(
            "No overlapping monthly history across ALL tickers (union set)."
        )

    # Run each strategy on that exact same window, restricting to the columns it needs
    runs = []
    for cfg in cfgs:
        needed_cols = list(dict.fromkeys([*cfg.universe, cfg.defensive, "SPY"]))
        px_use = px_monthly_common[needed_cols].dropna(how="any")
        # Ensure still same index (it should be, but keep safe)
        px_use = restrict_to_index(px_use, px_monthly_common.index)
        res = run_one_on_prices(cfg, px_use)
        runs.append(res)

    print("=" * 86)
    print("OEF vs XLG swap test (APPLIES SAME monthly window to both runs)")
    print("=" * 86)
    print(
        f"Common monthly window: {px_monthly_common.index.min().date()} -> {px_monthly_common.index.max().date()}"
    )
    print(f"Common months: {len(px_monthly_common)}")
    print()
    ...
    print("SUMMARY (both runs share the SAME monthly window):")
