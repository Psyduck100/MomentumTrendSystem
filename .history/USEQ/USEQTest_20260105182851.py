...
if __name__ == "__main__":
    ...
    pct = (counts / len(hold)).rename("pct_months")

    held_summary = pd.concat(
        [
            counts.rename("months_held"),
            pct,
        ],
        axis=1,
    )

    # Optional: first/last date held for each ticker
    first_last = hold.reset_index().rename(columns={"Date": "date", "holding": "ticker"})
    first_dates = first_last.groupby("ticker")["Date"].min().rename("first_held")
    last_dates = first_last.groupby("ticker")["Date"].max().rename("last_held")

    held_summary = held_summary.join(first_dates).join(last_dates)

    # Pretty formatting
    held_summary["pct_months"] = held_summary["pct_months"].map(lambda x: f"{x:.1%}")
    held_summary["first_held"] = held_summary["first_held"].dt.date
    held_summary["last_held"] = held_summary["last_held"].dt.date
