...
RebalanceFreq = Literal["D", "M"]  # Daily or Month-end
...
class StrategyConfig:
    ...
    # NEW: rebalance frequency
    rebalance_freq: RebalanceFreq = "D"  # "D" (daily) or "M" (month-end)
...
# ----------------------------
# Build positions (daily series, but monthly decisions if configured)
# ----------------------------
def build_positions(
    *,
    universe: list[str],
    ...
    - If cfg.rebalance_freq == "D": decide every day.
    - If cfg.rebalance_freq == "M": decide only on month-end trading days and hold until next month-end.
    ...
    if cfg.rebalance_freq == "D":
        ...
        raise ValueError("rebalance_freq must be 'D' or 'M'")

    # safer idxmax (future-proof): handle rows where all are NA
    all_na = scores.isna().all(axis=1)
    top = scores.idxmax(axis=1, skipna=True)
...
def main():
    ...
    for cfg in [cfg_daily, cfg_monthly]:
        # Compare two absolute filters for each rebalance frequency
    ...
    print("=" * 78)
    print("US EQUITIES MOMENTUM (blend_6_12) - DAILY vs MONTHLY REBALANCE COMPARISON")
    print("=" * 78)
