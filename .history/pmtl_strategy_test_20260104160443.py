...
4. GLD 12M return filter: hold GLD if 12M > 0, else IEF
5. GLD benchmark: hold GLD always
...
TRADING_50_TO_CALENDAR = int(50 * TRADING_TO_CALENDAR)
TRADING_100_TO_CALENDAR = int(100 * TRADING_TO_CALENDAR)
TRADING_200_TO_CALENDAR = int(200 * TRADING_TO_CALENDAR)
...
def backtest_50_trading_ma(prices: pd.DataFrame) -> dict:
    """Backtest: GLD above 50 trading-day MA -> GLD, else IEF."""
    prices["GLD_50TMA"] = prices["GLD"].rolling(window=TRADING_50_TO_CALENDAR).mean()
    ...
    for i in range(1, len(monthly)):
        ...
        gld_ma = monthly.iloc[i]["GLD_50TMA"]
...
def main():
    ...
    # Run all strategies
    ma50_result = backtest_50_trading_ma(prices)
    ...
    if all(len(r) > 0 for r in [ma50_ret, ma100_ret, ma200_ret, ret12m_ret, gld_ret]):
        ...
        print(f"Trading days: 50~{TRADING_50_TO_CALENDAR}cal, 100~{TRADING_100_TO_CALENDAR}cal, 200~{TRADING_200_TO_CALENDAR}cal")
        print("="*160)
