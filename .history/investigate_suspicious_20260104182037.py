...
print(
    f"  Periods in CASH: {(threeday_ema50 == 0).sum() / len(threeday_ema50) * 100:.1f}%"
)
...
for window in [50, 60, 70]:
    ...
    if col in threeday.columns:
        ...
        print(f"\n{col}:")
        print(
            f"  In CASH (0%): {zero_count} periods ({zero_count/len(ema_col)*100:.1f}%)"
        )
        print(
            f"  In GLD (≠0%): {nonzero_count} periods ({nonzero_count/len(ema_col)*100:.1f}%)"
        )
        if nonzero_count > 0:
            print(
                f"  Average GLD return when in position: {ema_col[ema_col != 0].mean():.4f}"
            )
