...
print("-" * 100)
print(
    f"{'Frequency':<15} {'Type':<5} {'Window':<7} {'CAGR':<12} {'Sharpe':<12} {'MaxDD':<12} {'Rebal/Year':<12}"
)
print("-" * 100)
...
print(
    f"{'Monthly':<15} {monthly_type:<5} {monthly_window:<7} {monthly_best['cagr']:>10.2%}  "
...
print(
    f"{'Biweekly':<15} {biweekly_type:<5} {biweekly_window:<7} {biweekly_best['cagr']:>10.2%}  "
...
print(
    f"{'Weekly':<15} {weekly_type:<5} {weekly_window:<7} {weekly_best['cagr']:>10.2%}  "
...
print(
    f"{'Every 3 days':<15} {threeday_type:<5} {threeday_window:<7} {threeday_best['cagr']:>10.2%}  "
...
print(
    f"   Biweekly:    {biweekly_best['cagr']:.2%}  ({biweekly_best['cagr'] - monthly_best['cagr']:+.2%} vs monthly)"
)
print(
    f"   Weekly:      {weekly_best['cagr']:.2%}  ({weekly_best['cagr'] - monthly_best['cagr']:+.2%} vs monthly)"
)
print(
    f"   Every 3 day: {threeday_best['cagr']:.2%}  ({threeday_best['cagr'] - monthly_best['cagr']:+.2%} vs monthly)"
)
...
THE BUG WAS:
  periods_per_year = {'ME': 12, 'W': 52, '2W': 26}.get(frequency, 12)
...
print("\nMONTHLY (ME):")
print("-" * 80)
monthly_top = monthly.nlargest(5, "cagr")[
    ["type", "window", "cagr", "sharpe", "max_drawdown"]
]
for idx, (_, row) in enumerate(monthly_top.iterrows(), 1):
    print(
        f"{idx}. {row['type']:3s} {int(row['window']):3d}: {row['cagr']:6.2%} CAGR, "
        f"{row['sharpe']:5.3f} Sharpe, {row['max_drawdown']:7.2%} MaxDD"
    )

print("\nBIWEEKLY (2W):")
print("-" * 80)
biweekly_top = biweekly.nlargest(5, "cagr")[
    ["type", "window", "cagr", "sharpe", "max_drawdown"]
]
...
print("\nWEEKLY (W):")
print("-" * 80)
weekly_top = weekly.nlargest(5, "cagr")[
    ["type", "window", "cagr", "sharpe", "max_drawdown"]
]
...
print("\nEVERY 3 DAYS (3D):")
print("-" * 80)
threeday_top = threeday.nlargest(5, "cagr")[
    ["type", "window", "cagr", "sharpe", "max_drawdown"]
]
