...
print("-" * 80)
print(f"{'Rank':<5} {'Type':<5} {'Window':<8} {'CAGR':<10} {'Sharpe':<10} {'MaxDD':<10}")
print("-" * 80)
for rank, (idx, row) in enumerate(df_results_sorted.head(5).iterrows(), 1):
    print(f"{rank:<5} {row['type']:<5} {int(row['window']):<8} {row['cagr']:>8.2%}  {row['sharpe']:>8.3f}  {row['max_drawdown']:>8.2%}")
...
print("PMTL with TB3MS (EMA 100):                     {:.2%} CAGR, {:.3f} Sharpe".format(
    best_strategy['cagr'], best_strategy['sharpe']))
print(f"Outperformance:                                 +{best_strategy['cagr']-0.1328:.2%} CAGR")
print(f"Sharpe Ratio Comparison:                        {best_strategy['sharpe']:.3f} vs 0.920")
