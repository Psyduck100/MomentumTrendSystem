...
def compute_blend_score(prices_df: pd.DataFrame) -> pd.Series:
    """Compute blend_6_12 score: 50% 6M + 50% 12M returns."""
    ret_6m = prices_df.pct_change(6)
    ret_12m = prices_df.pct_change(12)
...
def backtest_us_equities(prices: pd.DataFrame) -> dict:
    ...
    for i in range(1, len(monthly)):
        ...
        blend_scores = compute_blend_score(monthly).iloc[i]
        ret_12m = monthly.pct_change(12).iloc[i]

        # Get scores for US equities only (excluding IEF)
        us_blend = blend_scores[UNIVERSE]
        us_12m = ret_12m[UNIVERSE]
