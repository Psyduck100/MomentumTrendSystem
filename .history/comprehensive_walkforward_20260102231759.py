...
RETURNS_STORAGE_DIR = Path("walkforward_returns")
WINDOW_TICKER_CACHE: Dict[Tuple[str, str], List[str]] = {}


def _slugify(value: Any) -> str:
    """Convert arbitrary labels to filesystem-friendly slugs."""
    text = str(value)
    allowed = []
    for ch in text:
...
def persist_results(methodology_name: str, results: List[Dict[str, Any]], csv_path: Path) -> None:
    """Persist summary rows plus per-window return series for later statistical analysis."""

    if not results:
        print(f"DEBUG: No results to persist for {methodology_name}")
        return

    RETURNS_STORAGE_DIR.mkdir(exist_ok=True)
    method_dir = RETURNS_STORAGE_DIR / _slugify(methodology_name)
    method_dir.mkdir(parents=True, exist_ok=True)

    rows = []
    for idx, result in enumerate(results):
        row = {k: v for k, v in result.items() if k != "test_returns"}
        returns_series = result.get("test_returns")

        if returns_series is not None and not returns_series.empty:
            window_label = (
                result.get("test_year")
                or result.get("regime")
                or result.get("test_start")
                or f"window_{idx}"
            )
            slug = f"{idx:03d}_{_slugify(window_label)}"
            returns_path = method_dir / f"{slug}.csv"
            returns_series.rename("return").to_frame().to_csv(returns_path)
            row["returns_path"] = returns_path.as_posix()
        else:
            row["returns_path"] = None
    ...
    print(
        f"DEBUG: Persisted {len(df)} rows to {csv_path} and per-window returns to {method_dir}"
    )
...
def run_backtest_config(
    ...
    end_date: str,
    config: Dict[str, Any],
    lookback_buffer_months: int = 0,
) -> Dict[str, Any]:
    """Run backtest with specific configuration"""

    try:
        start_dt = pd.to_datetime(start_date)
        if lookback_buffer_months > 0:
            adjusted_start_dt = start_dt - pd.DateOffset(months=lookback_buffer_months)
        else:
            adjusted_start_dt = start_dt
        adjusted_start = adjusted_start_dt.strftime("%Y-%m-%d")
        cache_key = (adjusted_start, end_date)
        cached_tickers = WINDOW_TICKER_CACHE.get(cache_key)
        tickers_to_use = cached_tickers if cached_tickers else tickers
        ...
        usable_tickers = result.get("tickers", [])
        if usable_tickers and cache_key not in WINDOW_TICKER_CACHE:
            WINDOW_TICKER_CACHE[cache_key] = usable_tickers

        df = result["overall_returns"]
        if df.empty:
            return None

        window_returns = df.loc[start_date:end_date]
        if window_returns.empty:
            return None

        returns_series = window_returns["return"]
        ...
        turnover_count = 0
        prev_symbols = set()
        relevant_positions: List[List[str] | str] = []
        positions = result.get("overall_positions", [])
        if positions:
            try:
                locs = df.index.get_indexer(
                    [returns_series.index[0], returns_series.index[-1]]
                )
                if -1 in locs:
                    raise KeyError
                start_idx, end_idx = locs
                relevant_positions = positions[start_idx : end_idx + 1]
            except KeyError:
                relevant_positions = positions[-len(returns_series) :]

        for symbols in relevant_positions:
        ...
        denom = len(relevant_positions) if relevant_positions else len(returns_series)
        avg_monthly_turnover = (turnover_count / denom) if denom > 0 else 0
...
def anchored_walkforward(tickers, bucket_map, start_year=2015, end_year=2025):
    ...
    print(f"DEBUG: Generated {len(all_configs)} configs")

    # Start with 3-year initial training period
    initial_train_years = 3
    print(f"DEBUG: Will test years from {start_year + initial_train_years} to {end_year}")

    for test_year in range(start_year + initial_train_years, end_year + 1):
        print(f"DEBUG: Starting test_year={test_year}")
        ...
        for i, config in enumerate(all_configs):
            ...
            if train_metrics and train_metrics["cagr"] > best_train_cagr:
                best_train_cagr = train_metrics["cagr"]
        ...
        print(f"    Score Mode: {best_config['score_mode']}")
        print(f"    Filter: {best_config['filter_mode']} (band={best_config['filter_band']})")
        print(f"    Gap: {best_config['rank_gap']}")
        print(f"    Train CAGR: {best_train_metrics['cagr']*100:.2f}%")
        print(f"    Train Sharpe: {best_train_metrics['sharpe']:.3f}")
            ...
            results.append(
                {
                    ...
                    "score_mode": best_config["score_mode"],
                    "filter_mode": best_config["filter_mode"],
                    "filter_band": best_config["filter_band"],
    ...
    print(f"\nDEBUG: Anchored test completed with {len(results)} valid results")
    return results
...
def rolling_walkforward(
    ...
    all_configs = generate_configs()
    print(f"DEBUG: Generated {len(all_configs)} configs")
    print(f"DEBUG: Will test years from {start_year + train_years} to {end_year}")
    ...
    print(f"\nDEBUG: Rolling test completed with {len(results)} valid results")
    return results
...
def print_summary(test_name, results):
    ...
    for r in results:
        config = {
            ...
            "filter_band": r.get("filter_band"),
            "rank_gap": r.get("rank_gap"),
        }

        if None in config.values():
            try:
                parsed = literal_eval(r["config"])
                config.update(parsed)
            except Exception:
                pass

        if config["score_mode"] is not None:
            score_modes.append(config["score_mode"])
        if config["filter_mode"] is not None:
            filters.append(f"{config['filter_mode']} (band={config['filter_band']})")
        if config["rank_gap"] is not None:
            gaps.append(str(config["rank_gap"]))

    if score_modes:
        print(f"  Score Modes: {Counter(score_modes).most_common()}")
    if filters:
        print(f"  Filters: {Counter(filters).most_common()}")
    if gaps:
...
def main():
    ...
    try:
        print("DEBUG: Calling anchored_walkforward...")
        anchored_results = anchored_walkforward(
            tickers, bucket_map, start_year=2015, end_year=2025
        )
        print(f"DEBUG: anchored_walkforward returned {len(anchored_results) if anchored_results else 0} results")

        # Save results
        if anchored_results:
            print("DEBUG: Saving anchored results to CSV...")
            persist_results("anchored", anchored_results, Path("walkforward_anchored.csv"))
            print_summary("ANCHORED WALK-FORWARD", anchored_results)
        else:
            print("  WARNING: No anchored results generated")
            anchored_results = []
    except Exception as e:
        print(f"  ERROR in anchored walk-forward: {e}")
        import traceback
        traceback.print_exc()
        anchored_results = []
    ...
    try:
        ...
        if rolling5_results:
            persist_results(
                "rolling5y", rolling5_results, Path("walkforward_rolling5y.csv")
            )
            print_summary("ROLLING 5-YEAR WALK-FORWARD", rolling5_results)
        else:
            print("  WARNING: No rolling 5-year results generated")
            rolling5_results = []
    except Exception as e:
        print(f"  ERROR in rolling 5-year walk-forward: {e}")
        import traceback
        traceback.print_exc()
        rolling5_results = []
    ...
    try:
        ...
        if rolling4_results:
            persist_results(
                "rolling4y", rolling4_results, Path("walkforward_rolling4y.csv")
            )
            print_summary("ROLLING 4-YEAR WALK-FORWARD", rolling4_results)
        else:
            print("  WARNING: No rolling 4-year results generated")
            rolling4_results = []
    except Exception as e:
        print(f"  ERROR in rolling 4-year walk-forward: {e}")
        import traceback
        traceback.print_exc()
        rolling4_results = []
    ...
    try:
        ...
        if regime_results:
            persist_results("regime", regime_results, Path("walkforward_regime.csv"))
            print_summary("REGIME-SPLIT", regime_results)
        else:
            print("  WARNING: No regime results generated")
            regime_results = []
    except Exception as e:
        print(f"  ERROR in regime-split: {e}")
        import traceback
        traceback.print_exc()
        regime_results = []
