...
def compute_benchmark_annual_returns() -> dict[str, dict[int, float]]:
    print("Downloading SPY/QQQ monthly prices...")
    data = yf.download(
        BENCH_TICKERS,
    ...
    if isinstance(data.columns, pd.MultiIndex):
        available_levels = set(data.columns.get_level_values(0))
        price_field = "Adj Close" if "Adj Close" in available_levels else "Close"
        adj_close = data[price_field].copy()
    else:
        adj_close = data.copy()
    adj_close = adj_close.dropna(how="all")
    print("Computing annual returns from monthly data...")
...
def main() -> None:
    ...
    print("Writing updated sheets back to workbook...")
