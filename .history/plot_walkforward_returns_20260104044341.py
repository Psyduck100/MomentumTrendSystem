...
def plot_walkforward_curves(csv_paths: List[Path], output: Path) -> None:
    ...
    for row, csv_path in enumerate(csv_paths):
        ...
        ax_eq.plot(
            equity.index, equity.values, label=f"Equity ({label})", color="tab:blue"
        )
        ...
        ax_dd.fill_between(
            drawdown.index, drawdown.values * 100, color="tab:red", alpha=0.3
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Plot walk-forward stitched equity curves"
    )
