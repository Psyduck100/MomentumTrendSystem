...
def run_walk_forward() -> None:
    ...
    # Parameter grid - focused test of key parameters
    thresholds = [None]  # Threshold already proven irrelevant
    lookback_options = [6, 12]
    rank_gaps = [0, 2]
    vol_flags = [True, False]

    # Walk-forward folds: expanding window, 2y test (to accommodate 12M lookback)
    # For 2019-2024 (6 years): 3 folds with 2y test periods
    folds = [
        {
    ...
    print(
        f"\nRunning {len(folds)} walk-forward folds (expanding window / 2y test for 12M lookback)..."
    )
                ...
                for threshold in thresholds:
                    ...
                    for fold_idx, fold in enumerate(folds, 1):
                        ...
                        if backtest_data["overall_returns"].empty:
                            print(
                                f"  [Fold {fold_idx} {fold['test_start'][:4]}] vol_adj={vol_flag}, lookback={lookback}M, gap={rank_gap_setting}: NO DATA"
                            )
                            continue
    ...
    print("\n" + "=" * 100)
    print("WALK-FORWARD RESULTS (2019-2024, expanding window / 2y test, 3 folds)")
    print(
        "Columns: vol_adj, lookback, threshold, rank_gap | Median/Mean/Std of CAGR, Sharpe, MaxDD, Turnover"
    )
    print("=" * 100)
    ...
    for agg in aggregated:
        ...
        print(
            ...
            f"n={agg['n_folds']} | "
            f"CAGR med={agg['median_cagr']:.2%} mean={agg['mean_cagr']:.2%} std={agg['std_cagr']:.2%} | "
            f"Sharpe med={agg['median_sharpe']:.2f} mean={agg['mean_sharpe']:.2f} std={agg['std_sharpe']:.2f} | "
            f"MaxDD med={agg['median_maxdd']:.2%} mean={agg['mean_maxdd']:.2%} std={agg['std_maxdd']:.2%} | "
            f"Turnover med={agg['median_turnover']:.2%} mean={agg['mean_turnover']:.2%} std={agg['std_turnover']:.2%}"
        )

    print("\n" + "=" * 100)
    print(
        f"Top 15 combos by median Sharpe (across {len(folds)} folds, 2021-2024 test periods):"
    )
    print("=" * 100)
    for i, agg in enumerate(aggregated[:15], 1):
        thr_txt = "none" if agg["threshold"] is None else f"{agg['threshold']:.0%}"
        print(
