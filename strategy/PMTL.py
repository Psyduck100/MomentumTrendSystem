...
def describe_regime_snapshot(
    ...
    *,
    ...
    feature_params_down: Dict[str, Any],
    rule_params_down: Dict[str, Any],
    asof_up: pd.Timestamp,
    asof_down: pd.Timestamp,
) -> Dict[str, Any]:
    close = as_series(close, "Close")
    ts_up = pd.Timestamp(asof_up)
    ts_down = pd.Timestamp(asof_down)
    price_up = float(close.loc[ts_up])
    price_down = float(close.loc[ts_down])
    ...
    ma_entry_up = feat_up["ma_entry"].loc[ts_up]
    ma_exit_up = feat_up["ma_exit"].loc[ts_up]
    slope_up = feat_up["ma_slope_entry"].loc[ts_up]
    ch_high_up = feat_up["ch_high_entry"].loc[ts_up]
    ch_low_up = feat_up["ch_low_exit"].loc[ts_up]

    ma_entry_down = feat_down["ma_entry"].loc[ts_down]
    ma_exit_down = feat_down["ma_exit"].loc[ts_down]
    slope_down = feat_down["ma_slope_entry"].loc[ts_down]
    ch_low_down = feat_down["ch_low_exit"].loc[ts_down]

    up_entry = (
        (price_up > ma_entry_up)
        and (slope_up > slope_min_up)
        and (price_up > ch_high_up)
    )
    up_exit = (price_up < ch_low_up) or (price_up < ma_exit_up * (1.0 - ma_buf_up))

    down_entry = (
        (price_down < ma_entry_down)
        and (slope_down < -slope_min_down)
        and (price_down < ch_low_down)
    )
    down_exit = price_down > ma_exit_down * (1.0 + ma_buf_down)

    up_reason = (
        f"Up entry: price {_cmp_flag(price_up, float(ma_entry_up), '>')} "
        f"MA{feature_params_up['ma_len_entry']} ({_fmt_level(ma_entry_up)}), "
        f"slope {_fmt_level(slope_up)} vs min {slope_min_up:.3f}, "
        f"price {_cmp_flag(price_up, float(ch_high_up), '>')} "
        f"{feature_params_up['entry_len']}d high ({_fmt_level(ch_high_up)}). "
        f"Up exit: price {_cmp_flag(price_up, float(ch_low_up), '<')} "
    ...
    down_reason = (
        f"Down entry: price {_cmp_flag(price_down, float(ma_entry_down), '<')} "
        f"MA{feature_params_down['ma_len_entry']} ({_fmt_level(ma_entry_down)}), "
        f"slope {_fmt_level(slope_down)} vs min -{slope_min_down:.3f}, "
        f"price {_cmp_flag(price_down, float(ch_low_down), '<')} "
        f"{feature_params_down['exit_len']}d low ({_fmt_level(ch_low_down)}). "
        f"Down exit: price {_cmp_flag(price_down, float(ma_exit_down) * (1.0 + ma_buf_down), '>')} "
