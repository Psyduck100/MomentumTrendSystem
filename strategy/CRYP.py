...
def _ma_signal_or_off(
    price: pd.Series, length: int | None, buffer: float | None
) -> pd.Series:
    if length is None:
...
def _avg_ma_signal(
    price: pd.Series, lengths: tuple[int, int], buffer: float | None
) -> pd.Series:
...
def build_components_for_config(
    ...
    if exit_len is None:
        ...
        sma_exit = _avg_ma_signal(
            price, (lengths[0], lengths[1]), cfg["ma_exit_buffer"]
        )
    else:
...
def describe_signal_reason(
    ...
    if exit_trigger:
        action = "SELL"
        reason = f"{action_label}: exit trigger met. " f"{exit_detail}"
    elif entry_trigger:
        action = "BUY"
        reason = f"{action_label}: entry trigger met. " f"{entry_detail}"
    else:
        action = "HOLD"
        reason = (
...
def main() -> None:
    ...
    for label, payload in results.items():
        config_name = (
            "Primary (MA+Donchian)" if label == "primary" else "Alt (Donchian+MA)"
        )
