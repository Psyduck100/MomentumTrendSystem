...
def last_complete_month_end(daily_index: pd.DatetimeIndex) -> pd.Timestamp:
    """
    Returns the last completed month-end label relative to the last daily bar.

    Behavior:
    - If the last bar is on or after the *business* month-end day for that month,
      treat that month as complete and return its calendar month-end label.
      (This handles month-ends that fall on weekends.)
    - Otherwise, return the previous month-end.
    """
    if len(daily_index) == 0:
        raise ValueError("Empty price index.")
    last_day = pd.Timestamp(daily_index[-1])

    calendar_month_end = (last_day + pd.offsets.MonthEnd(0)).normalize()
    business_month_end = (last_day + pd.offsets.BMonthEnd(0)).normalize()

    if last_day.normalize() >= business_month_end:
...
def main():
    ...
    prices_monthly = result["prices_monthly"]

    asof_ts = pd.Timestamp(asof)
    winner = decision["recommendation"]
    gated_price = None
    current_price = None
    if winner in prices_monthly.columns and asof_ts in prices_monthly.index:
        gated_price = float(prices_monthly.loc[asof_ts, winner])
    if winner in prices_daily.columns and len(prices_daily.index):
        current_price = float(prices_daily[winner].iloc[-1])
    ...
    if gated_price is not None:
        print(f"Gated price ({winner} @ {asof}): {gated_price:.2f}")
